...
# Kasi Sports News v197 editorial retention UX
app = FastAPI(
...
def _seo_registry_pages():
    ...
    for c in COMPETITION_REGISTRY:
        ...
        rows.extend([
            ...
            {"path":f"{base}/today","pageType":"today","title":f"{name} Today | Kasi Sports News","description":f"{name} matches today, live scores, statistics and KasiScore AI intelligence."},
...
def _ssr_match_section(match: dict, path: str) -> tuple[str,str,str]:
    ...
    desc=f"Kasi Sports News analysis for {home} vs {away}: current bookmaker odds, KasiScore prediction, confidence, form, injuries and match intelligence."
...
async def _render_seo_document(path: str) -> str:
    ...
    # v197: SEO metadata/schema stay in <head>; the old visible SSR article is removed.
    _SSR_CACHE[key]=html
    return html
