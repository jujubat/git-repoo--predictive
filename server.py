...
# Kasi Sports News v215 editorial retention UX
app = FastAPI(
...
# ---------------------------------------------------------------------------
# Kasi Sports SQL data layer (v215)
...
# ---------------------------------------------------------------------------
SPORTS_DB_PATH=os.getenv("SPORTS_DB_PATH",str(Path(__file__).with_name("kasi_sports.db")))
SPORTS_DB_ENABLED=os.getenv("SPORTS_DB_ENABLED","1").strip().lower() not in {"0","false","no"}

def _sports_db():
    con=sqlite3.connect(SPORTS_DB_PATH,timeout=20,check_same_thread=False)
...
async def sports_db_status():
    ...
    with _sports_db() as db:
        return {"enabled":True,"path":Path(SPORTS_DB_PATH).name,
...
async def team_profile(team: str = Query(...), season: int = Query(0, ge=0)):
    ...
    if raw.isdigit() and SPORTS_DB_ENABLED:
        ...
        if sql_profile:
            ...
            return sql_profile
    cache_key=f"team-profile-v215:{raw.lower()}:{season}"
    ...
    _team_profile_page_cache[f"team-profile-v215:{team_id}:{season}"]=result
    return result
...
def _is_generic_news_image(url: str) -> bool:
    ...
    # Block only genuine Google branding/logo assets, not publisher images served via Google CDN
    return any(x in u for x in ("news.google.com","gstatic.com/images/branding","googleusercontent.com/images/branding","google_news","logo_google","favicon"))
...
def _generic_news_image(url: str) -> bool:
    u=str(url or "").lower()
    return (not u) or any(x in u for x in ("news.google.com","gstatic.com/images/branding","google.com/images/branding","googleusercontent.com/images/branding","googlenews","google_news"))
...
async def _render_seo_document(path: str) -> str:
    ...
    # v215: SEO metadata/schema stay in <head>; the old visible SSR article is removed.
    _SSR_CACHE[key]=html
    return html
