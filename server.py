...
async def oddspapi_diagnostics():
    ...
    # v43: make diagnostics deterministic after a Render restart. If the
    # in-memory fixture cache is cold, reuse the existing /fixtures loader to
    # populate today's/upcoming API-Football fixtures, then inspect the cache.
    # This does not introduce a second API-Football implementation.
    mapping_warmed = False
    mapping_warm_error = None
    if not candidates and AFOOT_API_KEY:
        try:
            today_local = datetime.now().strftime("%Y-%m-%d")
            tomorrow_local = (datetime.now() + timedelta(days=1)).strftime("%Y-%m-%d")
            await fixtures(
                league="ALL",
                type="upcoming",
                range=None,
                date_from=today_local,
                date_to=tomorrow_local,
                refresh=0,
            )
            candidates = _oddspapi_cached_afoot_candidates()
            mapping_warmed = True
        except Exception as exc:
            mapping_warm_error = f"{type(exc).__name__}: {str(exc)[:160]}"
            report["errors"].append(f"API-Football mapping warm-up failed: {mapping_warm_error}")

    report["mapping"]["cacheWarmAttempted"] = bool(mapping_warmed or mapping_warm_error)
    report["mapping"]["cacheWarmSucceeded"] = bool(mapping_warmed and candidates)
    ...
    report["mapping"]["candidateSource"] = "existing-cache" if not mapping_warmed else "existing-fixtures-loader"
