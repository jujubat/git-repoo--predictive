...
async def fixture_live_bundle(fixture_id: int):
    ...
    fd,sd,ld,pd=await asyncio.gather(
...
async def team_profile(team: str = Query(...), season: int = Query(0, ge=0)):
    """Complete football team page with a six-hour assembled-page cache.
    Numeric provider IDs avoid a team-search call and make navigation resilient
    when the local provider budget is busy.
    """
    season = season or _current_season()
    cache_key = f"team-profile:{str(team).strip().lower()}:{season}"
    ...
    try:
        ...
        # If a name lookup is rate-limited, return a useful shell instead of a 429 page.
        if exc.status_code == 429:
            return {"team":{"id":None,"name":str(team)},"venue":{},"players":[],"past10":[],"future":[],"standings":[],"league":{},"season":season,"form":[],"goalsFor":None,"goalsAgainst":None,"teamStats":{},"injuries":[],"nextOdds":{},"source":"KasiScore cache pending","temporarilyLimited":True,"updatedAt":datetime.now(timezone.utc).isoformat()}
        raise
    ...
    _team_profile_page_cache[f"team-profile:{team_id}:{season}"]=result
    return result
...
async def ai_predictions(
    ...
    display_predictions = eligible[:limit] if eligible else sorted(model_only, key=lambda x: x.get("prediction",{}).get("confidence",0), reverse=True)[:limit]
    return {
...
async def sports_news(sport: str = Query("all"), q: str = Query(""), limit: int = Query(30, ge=1, le=50)):
    ...
    Football is intentionally restricted to Metro Football + GOAL South Africa.
    Other sports keep the existing South-Africa-focused Google News discovery.
    Article clicks stay inside KasiScore; /sports/news/article returns only
    publisher metadata/summary/media, not a copied full article.
    """
    ...
    if sport_key == "all":
        # Build the dashboard feed from three independent searches so football
        # restrictions do not accidentally filter rugby/cricket out.
        searches = [
            ("football", f"({BIG_CLUBS} OR South Africa football) (site:metro.co.uk/sport/football OR site:goal.com/en-za)"),
            ("rugby", "South Africa rugby Springboks URC Bulls Stormers Sharks Lions Currie Cup"),
            ("cricket", "South Africa cricket Proteas SA20 cricket"),
        ]
        parts = await asyncio.gather(*[_google_news(qry, max(4, limit//3+2)) for _,qry in searches], return_exceptions=True)
        merged=[]; seen=set()
        for (sp,_), rows in zip(searches, parts):
            if isinstance(rows, Exception):
                log.warning("News %s failed: %s", sp, rows); continue
            for x in rows:
                if sp=="football":
                    src=(str(x.get("source") or "")+" "+str(x.get("title") or "")).lower()
                    if "goal" not in src and "metro" not in src: continue
                key=(x.get("link") or x.get("title") or "").strip().lower()
                if not key or key in seen: continue
                seen.add(key); y=dict(x); y["sport"]=sp; y["publisher"]=y.get("source") or sp.title(); merged.append(y)
        merged.sort(key=lambda x: x.get("published") or "", reverse=True)
        return {"sport":"all","count":len(merged[:limit]),"items":merged[:limit],"source":"Metro/GOAL football + South Africa rugby/cricket news"}
    terms={
        "all":f"({BIG_CLUBS} OR South Africa football) (site:metro.co.uk/sport/football OR site:goal.com/en-za)",
        "football":f"({BIG_CLUBS} OR South Africa football transfers injuries) (site:metro.co.uk/sport/football OR site:goal.com/en-za)",
    ...
    if sport_key in ("football","all"):
        # Keep requested search terms, but never broaden football beyond the two approved publishers.
        user_q=q.strip()
        query=(f"({user_q}) " if user_q else "") + terms[sport_key]
    else:
        query=q.strip() or terms.get(sport_key, sport)
    try:
        items=await _google_news(query, limit)
        if sport_key in ("football","all"):
            def approved(x):
                src=str(x.get("source") or "").lower()
                title=str(x.get("title") or "").lower()
                return ("goal" in src or "metro" in src or "goal.com" in title or "metro" in title)
            items=[x for x in items if approved(x)]
        for x in items:
            x["publisher"] = x.get("source") or "Football News"
            x["video"] = x.get("video") or ""
        return {"sport":sport,"query":query,"count":len(items),"items":items,"source":"Metro Football + GOAL South Africa" if sport_key in ("football","all") else "Google News RSS"}
    except Exception as exc:
        log.warning("News provider unavailable: %s", exc)
        return {"sport":sport,"query":query,"count":0,"items":[],"source":"Metro Football + GOAL South Africa" if sport_key in ("football","all") else "Google News RSS","error":str(exc)}
