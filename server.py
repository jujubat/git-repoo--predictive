...
class KasiCanonicalHostMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        if scope.get("type")=="http":
            ...
            if host in {"www.kasilivescore.com","git-repoo-predictive.onrender.com"}:
                path=scope.get("path") or "/"
                qs=scope.get("query_string",b"").decode("latin1")
                target=KASI_PUBLIC_ORIGIN+path+("?"+qs if qs else "")
...
app.add_middleware(KasiCanonicalHostMiddleware)
