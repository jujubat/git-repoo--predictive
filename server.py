...
def _auth_db():
    ...
    if TURSO_DB_URL:
        try:
            ...
            # libsql_client uses the same DB-API 2.0 interface as sqlite3
            c = libsql_client.connect(
                url=TURSO_DB_URL,
                auth_token=TURSO_DB_TOKEN or None,
            )
        except ImportError:
            raise RuntimeError(
                "TURSO_DB_URL is set but libsql-client is not installed. "
                "Add 'libsql-client' to requirements.txt and redeploy."
            )
    else:
        AUTH_DB.parent.mkdir(parents=True, exist_ok=True)
        c = sqlite3.connect(AUTH_DB, timeout=20)
...
def auth_login(p: AuthPayload, request: Request):
    ...
    configured_admin=os.getenv("ADMIN_EMAIL",os.getenv("PAYMENT_ADMIN_EMAIL","")).strip().lower()
    ...
    # Persist the configured administrator's entitlement after successful
    # password authentication. This never bypasses password verification.
    if is_configured_admin and (effective_role != str(row[6] or "user").lower() or not bool(row[3])):
        c=_auth_db()
        try:
            c.execute("UPDATE users SET pro=1, role='admin' WHERE id=?",(row[0],))
            c.commit()
        finally:
            c.close()

    return {"token":_token(row[0]),"email":row[1],"username":row[4],"phone":row[5],
            "pro":effective_pro,"role":effective_role,"admin":effective_role=="admin"}
...
def get_current_user(authorization: Optional[str] = Header(default=None)):
    ...
    configured_admin=os.getenv("ADMIN_EMAIL",os.getenv("PAYMENT_ADMIN_EMAIL","")).strip().lower()
    is_configured_admin=bool(configured_admin and str(row[1]).strip().lower()==configured_admin)
    role="admin" if is_configured_admin else str(row[3] or "user").lower()
    pro=True if is_configured_admin else bool(row[2])
    return {"id":int(row[0]),"email":str(row[1]),"pro":pro,
            "role":role,"status":str(row[4] or "active").lower()}
