...
LEAGUE_IDS = {
    ...
    "Nigeria Premier Football League": 332, "Ghana Premier League": 570, "Saudi Pro League": 307, "UAE Pro League": 301,
...
async def fixture_stats(fixture_id: int):
    ...
    data = await _afoot_get("/fixtures/statistics", {"fixture": fixture_id}, force_fresh=True)
...
async def fixture_live_bundle(fixture_id: int):
    ...
    fd,sd,ld=await asyncio.gather(
        _afoot_get("/fixtures", {"id":fixture_id}, force_fresh=True),
...
async def sports_videos(
    ...
    search = q.strip() or {
        "football": "football match highlights South Africa Premier League PSL",
        "rugby": "rugby match highlights Springboks URC",
        "cricket": "cricket match highlights Proteas",
...
async def home_news(limit: int = Query(24, ge=6, le=60)):
    ...
    feeds=[
        ("GOAL", "https://www.goal.com/en-us?fmt=rss&id=251"),
        ("Metro", "https://metro.co.uk/sport/football/feed/"),
        ("Soccer Laduma", "https://www.soccerladuma.co.za/feed/"),
    ]
    per=max(6,min(16,limit//3+2))
    async def load(label,url):
        try:
            return await _publisher_rss(url,label,per)
        except Exception as exc:
            log.debug("Publisher RSS %s unavailable: %s",label,exc)
            query={"GOAL":"site:goal.com/en-za football","Metro":"site:metro.co.uk/sport/football football","Soccer Laduma":"site:soccerladuma.co.za football"}[label]
            try:
    ...
    return {"count":len(items[:limit]),"items":items[:limit],"sources":[x[0] for x in feeds],"updatedAt":datetime.now(timezone.utc).isoformat()}
...
async def team_profile(team: str = Query(...), season: int = Query(0, ge=0)):
    ...
    return {"team":team_obj,"venue":p.get("venue") or {},"players":players,"past10":norm(recent.get("response") or []),"future":norm(future.get("response") or []),"standings":standings,"league":({"id":league_hint[0],"season":league_hint[1]} if league_hint else {}),"season":season,"source":"API-Football","updatedAt":datetime.now(timezone.utc).isoformat()}
