...
# Kasi Sports News v201 editorial retention UX
app = FastAPI(
...
async def _google_news(query: str, limit: int = 20):
    ...
    for item in root.findall(".//item")[:limit]:
        ...
        items.append({"title":title,"link":link,"publisherLink":publisher_link,"published":pub,"source":src,"image":thumb,"imageOrigin":"rss" if thumb else ""})
...
async def sports_news(sport: str = Query("all"), q: str = Query(""), limit: int = Query(30, ge=1, le=50)):
    ...
    selected=await asyncio.gather(*[enrich_one(x) for x in selected])
...
async def _render_seo_document(path: str) -> str:
    ...
    # v201: SEO metadata/schema stay in <head>; the old visible SSR article is removed.
    _SSR_CACHE[key]=html
    return html
