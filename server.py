...
# Kasi Sports News v208 editorial retention UX
app = FastAPI(
...
def auth_login(p: AuthPayload):
    ...
    return {"token":_token(row[0]),"email":row[1],"username":row[4],"phone":row[5],"pro":bool(row[3]),"role":row[6]}
...
def auth_me(authorization: Optional[str]=Header(default=None)):
    ...
    return {"id":row[0],"email":row[1],"pro":bool(row[2]),"role":row[3]}
...
async def public_route(kind:str,provider_id:int,name:str=Query("")):
...
async def directory_search(q:str=Query(...,min_length=2,max_length=80),season:int=Query(0,ge=0)):
    season=season or _current_season();q=q.strip();key=f"search:{q.lower()}:{season}";c=_PLAYERS_EXPLORER_CACHE.get(key)
    if c:return c
    async def safe(path,params):
        try:return await _afoot_get(path,params)
        except Exception:return {"response":[]}
...
async def team_profile(team: str = Query(...), season: int = Query(0, ge=0)):
    """Fast team page first paint.

    Returns identity, badge, venue, squad, recent/upcoming fixtures and a
    league table when available. Expensive stats/injuries/odds enrichment is
    deliberately deferred so a team click does not fan out into many provider
    requests or fail with 429/timeouts.
    """
    season = season or _current_season()
    raw = str(team).strip()
    cache_key = f"team-profile-fast:{raw.lower()}:{season}"
    ...
    async def safe(path, params):
        try:
            ...
            log.debug("Team page %s failed for %s: %s", path, team_id, exc)
            return {"response":[]}

    profile, squad, recent, future = await asyncio.gather(
        safe("/teams", {"id":team_id}),
        safe("/players/squads", {"team":team_id}),
        safe("/fixtures", {"team":team_id, "season":season, "last":8}),
        safe("/fixtures", {"team":team_id, "season":season, "next":8}),
    )

    p = (profile.get("response") or [{}])[0]
    team_obj = p.get("team") or {"id":team_id, "name":team_name}
    if not team_obj.get("id"):
        team_obj["id"] = team_id
    if not team_obj.get("logo") and team_id:
        team_obj["logo"] = f"/sports/football/team-badge/{team_id}"

    players = []
    for sr in squad.get("response") or []:
        for pl in sr.get("players") or []:
            players.append({
                "id":pl.get("id"), "name":pl.get("name"), "age":pl.get("age"),
                "number":pl.get("number"), "position":pl.get("position"),
                "photo":pl.get("photo"), "nationality":pl.get("nationality")
            })

    def norm(items):
        out = []
        for f in items:
            ts=f.get("teams") or {}; g=f.get("goals") or {}
            fs=f.get("fixture") or {}; lg=f.get("league") or {}
            out.append({
                "id":fs.get("id"), "date":fs.get("date"),
                "status":(fs.get("status") or {}).get("short"),
                "statusLong":(fs.get("status") or {}).get("long"),
                "home":(ts.get("home") or {}).get("name"),
                "away":(ts.get("away") or {}).get("name"),
                "homeId":(ts.get("home") or {}).get("id"),
                "awayId":(ts.get("away") or {}).get("id"),
                "homeLogo":(ts.get("home") or {}).get("logo"),
                "awayLogo":(ts.get("away") or {}).get("logo"),
                "homeScore":g.get("home"), "awayScore":g.get("away"),
                "league":lg.get("name"), "leagueId":lg.get("id"),
    ...
    for fx in (future.get("response") or []) + (recent.get("response") or []):
        lg = fx.get("league") or {}
        if lg.get("id"):
            league_hint = (int(lg["id"]), int(lg.get("season") or season))
            break
    ...
    if league_hint:
        try:
            ...
            log.debug("Team standings deferred/failed for %s: %s", team_id, exc)

    form=[]; gf=ga=0
    for x in reversed(past):
        hs=int(x.get("homeScore") or 0); av=int(x.get("awayScore") or 0)
        if x.get("homeId")==team_id:
            gf+=hs; ga+=av; r="W" if hs>av else "D" if hs==av else "L"
        else:
            gf+=av; ga+=hs; r="W" if av>hs else "D" if av==hs else "L"
        form.append(r)
    form=form[-5:]

    result = {
        "team":team_obj, "venue":p.get("venue") or {}, "players":players,
        "past10":past, "future":upcoming, "standings":standings,
        "league":({"id":league_hint[0],"season":league_hint[1]} if league_hint else {}),
        "season":season, "form":form, "goalsFor":gf, "goalsAgainst":ga,
        "teamStats":{}, "injuries":[], "nextOdds":{},
        "detailsDeferred":True,
        "source":"API-Football · fast cached team profile",
        "updatedAt":datetime.now(timezone.utc).isoformat()
    }
    _team_profile_page_cache[cache_key] = result
    _team_profile_page_cache[f"team-profile-fast:{team_id}:{season}"] = result
    return result
...
async def player_profile(player: int = Query(..., ge=1), season: int = Query(0, ge=0)):
    ...
    pobj=profile.get("player") or {"id":player}
    ...
    result={
        ...
        "season":season,
        "detailsDeferred":True,
        "source":"API-Football · fast cached player profile",
        "updatedAt":datetime.now(timezone.utc).isoformat(),
    }
...
async def ai_predictions(
    ...
    for match in fixtures_data.get("matches", []):
        ...
        if not _valid_1x2(match.get("odds", {})):
            ...
            model_only.append({**match, **pred_result, "oddsAvailable":False, "bookmakerGatePassed":False, "modelOnly":True, "predictionBasis":"available team/statistical data; no current bookmaker 1X2"})
            continue
...
def _is_generic_news_image(url: str) -> bool:
    u=(url or "").lower()
    return (not u) or any(x in u for x in ("news.google","gstatic","googleusercontent","google_news","favicon","branding","logo_google"))
...
async def sports_news_image(url: str = Query(...)):
    """Same-origin proxy for publisher news images; cached by the publisher/CDN and browser."""
    try:
        p=urllib.parse.urlparse(url)
        host=(p.hostname or "").lower()
        if p.scheme not in {"http","https"}: raise HTTPException(status_code=400,detail="Invalid image URL")
        if _is_generic_news_image(url) or host in {"localhost","127.0.0.1","::1"}: raise HTTPException(status_code=400,detail="Invalid image source")
        c=await _sports_client()
        r=await c.get(url,follow_redirects=True,timeout=httpx.Timeout(6,connect=3),headers={"User-Agent":"Mozilla/5.0 KasiScore/194","Referer":f"{p.scheme}://{p.netloc}/"})
        r.raise_for_status()
        ct=r.headers.get("content-type","image/jpeg")
        if not ct.startswith("image/"): raise HTTPException(status_code=415,detail="Not an image")
        if len(r.content)>5_000_000: raise HTTPException(status_code=413,detail="Image too large")
        return Response(content=r.content,media_type=ct,headers={"Cache-Control":"public, max-age=21600"})
...
async def _enrich_news_card(item: dict) -> dict:
    """Return a visible news card with a real publisher image whenever one is exposed."""
    out=dict(item or {})
    if _generic_news_image(out.get("image")):out["image"]=""
    original=str(out.get("link") or "").strip()
    if not original:return out
    direct=str(out.get("publisherLink") or "").strip()
    resolved=direct if direct.startswith(("http://","https://")) else await _resolve_news_publisher_url(original,out.get("source") or out.get("publisher") or "")
    key=f"news-enriched-v195:{resolved}"
    cached=NEWS_CACHE.get(key)
    if cached:
        merged=dict(out);merged.update(cached);return merged
    try:
        c=await _sports_client()
        r=await c.get(resolved,follow_redirects=True,timeout=httpx.Timeout(7,connect=3),
            headers={"User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/124 Safari/537.36",
        ...
        if not image:
            for pat in (
                ...
                if m:image=unescape(m.group(1));break
        if image.startswith("//"):image="https:"+image
        elif image.startswith("/"):
            p=urllib.parse.urlparse(final);image=f"{p.scheme}://{p.netloc}{image}"
        if _generic_news_image(image):image=""
        payload={
            "image":image or out.get("image",""),
            "summary":summary or out.get("summary",""),
            "resolvedUrl":final,
            "publisherUrl":final,
            "publisherTitle":title or out.get("title",""),
            "imageOrigin":"publisher-meta" if image else out.get("imageOrigin",""),
            "imageAvailable":bool(image or out.get("image"))
        }
        NEWS_CACHE[key]=payload;out.update(payload)
    except Exception as exc:
...
async def sports_news(sport: str = Query("all"), q: str = Query(""), limit: int = Query(30, ge=1, le=50)):
    """Fail-soft sports headline discovery for the dashboard.

    Football searches GOAL South Africa and Metro independently instead of one
    over-constrained Google News query. Rugby and cricket use South-African
    discovery terms. Empty/failed source searches never blank the whole widget.
    """
    sport_key=sport.lower().strip()
    if sport_key=="all":
        searches=[
            ("football","GOAL.com South Africa football"),
            ("football","Metro.co.uk football"),
            ("rugby","South Africa rugby Springboks URC Bulls Stormers Sharks Lions"),
            ("cricket","South Africa cricket Proteas SA20"),
        ]
    elif sport_key=="football":
        searches=[("football",(q.strip()+" " if q.strip() else "")+"GOAL.com South Africa football"),
                  ("football",(q.strip()+" " if q.strip() else "")+"Metro.co.uk football")]
    elif sport_key=="rugby":
        searches=[("rugby",q.strip() or "South Africa rugby Springboks URC Bulls Stormers Sharks Lions")]
    elif sport_key=="cricket":
        searches=[("cricket",q.strip() or "South Africa cricket Proteas SA20")]
    else:
        searches=[(sport_key,q.strip() or f"South Africa {sport_key}")]

    parts=await asyncio.gather(*[_google_news(query,max(6,limit)) for _,query in searches],return_exceptions=True)
    merged=[];seen=set()
    for (sp,_),rows in zip(searches,parts):
        if isinstance(rows,Exception):
            log.warning("News discovery failed for %s: %s",sp,rows);continue
        for x in rows:
            src=(str(x.get("source") or "")+" "+str(x.get("title") or "")).lower()
            if sp=="football" and not any(k in src for k in ("goal","metro")):
                continue
            key=(x.get("link") or x.get("title") or "").strip().lower()
            if not key or key in seen: continue
            seen.add(key)
            y=dict(x);y["sport"]=sp;y["publisher"]=y.get("source") or ("GOAL / Metro" if sp=="football" else sp.title());y["video"]=y.get("video") or ""
            merged.append(y)
    ...
    try:
        selected=await asyncio.wait_for(asyncio.gather(*[enrich_one(x) for x in selected]), timeout=5.5)
    except asyncio.TimeoutError:
        # Return discovered headlines immediately; image enrichment remains best-effort/cached.
        selected=[dict(x) for x in selected]
    return {"sport":sport_key,"count":len(selected),"items":selected,"source":"GOAL South Africa + Metro Football + SA rugby/cricket discovery","partial":any(isinstance(x,Exception) for x in parts)}
...
async def _ssr_generic_section(path: str) -> tuple[str,str,str]:
    p=path.rstrip("/") or "/"
    if p=="/today": title="Today’s Sports Intelligence"; desc="Today’s fixtures, live matches, predictions, results and Kasi Sports News sports intelligence."
    elif p=="/weekend": title="Weekend Sports Intelligence"; desc="Weekend fixtures, predictions, live matches, results and Kasi Sports News football intelligence."
    elif p.startswith("/football/"): name=p.split("/")[2].replace("-"," ").title(); title=f"{name} Football Intelligence"; desc=f"{name} fixtures, results, predictions, table and Kasi Sports News match intelligence."
    else: title="Kasi Sports News"; desc="Live scores, predictions, odds analysis and sports intelligence."
    body=f'<article class="kasiscore--page" data--page="hub"><nav aria-label="Breadcrumb"><a href="/">Kasi Sports News</a> / {_ssr_text(title)}</nav><h1>{_ssr_text(title)}</h1><p class="-lead">{_ssr_text(desc)}</p><p>Kasi Sports News combines live sports data, bookmaker market information and predictive intelligence. The interactive application hydrates on this same URL for live updates and deeper analysis.</p><p><a href="/today">Today</a> · <a href="/weekend">Weekend</a> · <a href="/football">Football</a></p></article>'
...
async def _render_seo_document(path: str) -> str:
    ...
    # v208: SEO metadata/schema stay in <head>; the old visible SSR article is removed.
    _SSR_CACHE[key]=html
    return html
...
async def ssr_admin():
    return HTMLResponse(await _render_seo_document("/admin"),headers={"Cache-Control":"no-cache"})
...
def _require_admin_user(authorization: Optional[str]):
    u=_user_from_token(authorization)
    ...
    if str(u[3] or "user").lower()!="admin" and str(u[1]).lower()!=configured: raise HTTPException(403,"Admin access required")
    return u
