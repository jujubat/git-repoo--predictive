...
async def sport_match_details(sport:str,match_id:str,league:str=Query("")):
    ...
    if sport=="football":
        ...
        fd,stats,lineups,events,insights=await asyncio.gather(
            _afoot_get("/fixtures",{"id":fid}), fixture_stats(fid), fixture_lineups(fid),
            fixture_events(fid), fixture_insights(fid), return_exceptions=True)
        provider_error=""
        if isinstance(fd,Exception):
            provider_error=str(fd); log.warning("Football fixture unavailable fixture=%s: %s",fid,fd); f={}
        else:
            rr=fd.get("response") or [] if isinstance(fd,dict) else []; f=rr[0] if rr else {}
        stats=stats if isinstance(stats,dict) else {}; lineups=lineups if isinstance(lineups,dict) else {}
        events=events if isinstance(events,dict) else {}; insights=insights if isinstance(insights,dict) else {}
        partial=bool(f or stats.get("statistics") or lineups.get("teams") or events.get("events") or insights)
        return {"sport":"football","matchId":fid,"match":f,"statistics":stats.get("statistics",[]),
                ...
                "source":"API-Football","available":bool(f),"partialAvailable":partial,
                "warning":"Match header temporarily unavailable; available statistics are shown." if provider_error else "",
                "updatedAt":datetime.now(timezone.utc).isoformat()}
...
async def sports_news(
    ...
    if verified:
        ...
        qualified=await _news_quality_filter(pool,max(24,limit+offset))
        if qualified:
            _NEWS_VERIFIED_SNAPSHOT["items"]=qualified
            _NEWS_VERIFIED_SNAPSHOT["updated_at"]=time.time()
            selected=qualified[offset:offset+limit]
        else:
            selected=[]
...
async function J(u){{let last;for(let n=0;n<2;n++){{const c=new AbortController(),t=setTimeout(()=>c.abort(),7000);try{{const r=await fetch(u,{{signal:c.signal,headers:{{Accept:'application/json'}},cache:'no-store'}});if(r.ok)return await r.json();last=Error('HTTP '+r.status)}}catch(e){{last=e}}finally{{clearTimeout(t)}}if(n===0)await new Promise(r=>setTimeout(r,450))}}throw last||Error('Data temporarily unavailable')}}
