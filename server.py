...
_live_cache = TTLCache(maxsize=50, ttl=60)  # v184: one upstream live refresh per minute
...
# Kasi Sports News v206 editorial retention UX
app = FastAPI(
...
async def player_profile(player: int = Query(..., ge=1), season: int = Query(0, ge=0)):
    ...
    """
    season = season or _current_season()
    cache_key=f"player-profile-rich:{player}:{season}"
    cached=_team_cache.get(cache_key)
    if cached is not None:
        return cached
    ...
    result={
        ...
        "season": season, "source": "API-Football · rich cached profile", "updatedAt": datetime.now(timezone.utc).isoformat(),
    }
    _team_cache[cache_key]=result
    return result
...
async def _render_seo_document(path: str) -> str:
    ...
    # v206: SEO metadata/schema stay in <head>; the old visible SSR article is removed.
    _SSR_CACHE[key]=html
    return html
