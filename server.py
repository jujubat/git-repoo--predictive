...
def _auth_db():
    ...
    for col,typ in [("reset_token","TEXT"),("reset_expires","REAL")]:
...
_team_cache = TTLCache(maxsize=500, ttl=604800)     # 7 days (was 6 hrs) — team metadata is static
...
_rate_bucket = []
RATE_LIMIT_PER_MINUTE = int(os.getenv("RATE_LIMIT_PER_MINUTE", "30"))  # 30 req/min default; 0 to disable
...
def _normalise_odds(bookmakers: list[dict], live: bool = False) -> dict:
    ...
    preferred = [
        "Pinnacle", "Bet365", "Betway", "1xBet", "William Hill",
...
async def lifespan(app: FastAPI):
    v141_learning.init()
    log.info("Football Server v162 starting on port %d", PORT)
...
def auth_register(p: AuthPayload):
    ...
    c=_auth_db()
    try:
        ph=_pw(p.password); cur=c.execute("INSERT INTO users(email,password_hash,created_at) VALUES(?,?,?)",(email,ph,datetime.now(timezone.utc).isoformat())); c.commit()
        return {"token":_token(cur.lastrowid),"message":"Account created","pro":False}
    except sqlite3.IntegrityError: raise HTTPException(409,"Account already exists")
    finally:c.close()

@app.post("/auth/login")
def auth_login(p: AuthPayload):
    c=_auth_db(); row=c.execute("SELECT id,email,password_hash,pro FROM users WHERE email=?",(p.email.strip().lower(),)).fetchone(); c.close()
    if not row: raise HTTPException(401,"Invalid email or password")
    salt,stored=row[2].split("$",1)
    if not hmac.compare_digest(_pw(p.password,salt).split("$",1)[1],stored): raise HTTPException(401,"Invalid email or password")
    return {"token":_token(row[0]),"email":row[1],"pro":bool(row[3])}
...
def auth_forgot_password(p: PasswordResetPayload):
    email=p.email.strip().lower(); c=_auth_db(); row=c.execute("SELECT id FROM users WHERE email=?",(email,)).fetchone()
    if not row: c.close(); return {"message":"If the account exists, recovery instructions have been sent."}
    token=secrets.token_urlsafe(32); c.execute("UPDATE users SET reset_token=?,reset_expires=? WHERE id=?",(token,time.time()+1800,row[0])); c.commit(); c.close()
    result={"message":"If the account exists, recovery instructions have been sent."}
...
@app.get("/football-proxy")
async def football_proxy(request: Request):
    """Same-origin replacement for the old Netlify football.js proxy.
    Frontend calls /football-proxy?path=/teams&search=... exactly as it
    used to call /.netlify/functions/football?path=/teams&search=...
    """
    params = dict(request.query_params)
    path = params.pop("path", "/status")
    if not path.startswith("/"):
        path = "/" + path
    try:
        data = await _afoot_get(path, params)
...
async def fixtures(
    ...
    cache_key = f"fixtures:{league}:{type}:{bucket}"
...
async def fixture_stats(fixture_id: int):
    """Live/current match statistics for both fixtures and live matches."""
...
async def fixture_insights(fixture_id: int):
    ...
    tasks = {}
    if home_id:
        tasks["home_form"] = _fetch_team_form_detailed(home_id, league_id, season, 5)
        tasks["home_injuries"] = _afoot_get("/injuries", {"team": home_id, "season": season})
    if away_id:
        tasks["away_form"] = _fetch_team_form_detailed(away_id, league_id, season, 5)
    ...
    bookmaker_options = []
    selected_odds = {}
    preferred = {"Pinnacle": 0, "Bet365": 1, "Betway": 2, "1xBet": 3, "William Hill": 4,
    ...
    return {
        ...
        "form": {
            "home": {"team": home, "last5": result_rows(home_form, home_id)},
            "away": {"team": away, "last5": result_rows(away_form, away_id)},
        },
        ...
        try:
            ...
            if league.upper() == "ALL":
                ...
                try:
                    ...
                    lids = [lid for name in SCAN_LEAGUES if (lid := LEAGUE_IDS.get(name))]
            fetched = await _fetch_prematch_odds_by_dates(dates, league_ids=lids)
            real_by_id.update(fetched)
        except HTTPException as exc:
            log.warning("Pre-match odds fetch failed: %s", exc.detail)
...
async def odds(
    ...
    data = await _afoot_get("/odds", {"fixture": fixture_id})
    raw = data.get("response", [])

    norm = (
        _normalise_odds(raw[0].get("bookmakers", []))
        if raw
        else {}
    )
    valid = _valid_1x2(norm)
    ...
    result = {
        "fixture_id": fixture_id,
        "odds": norm if valid else {},
        "source": "API-Football" if valid else "Unavailable",
        "available": valid,
        "bookmakerGatePassed": valid,
        "quota": dict(_quota_state),
    }
...
async def odds_batch(
    ...
    try:
        ids = [
            int(x.strip())
            for x in fixture_ids.split(",")
            if x.strip()
        ]
    except ValueError:
        raise HTTPException(
            400,
            "fixture_ids must be comma-separated integers",
        )
    ...
    if len(ids) > 100:
        raise HTTPException(
            400,
            "Maximum 100 fixture IDs per batch request",
        )
    ...
    return {
        "odds": result,
        "count": len(result),
        "fetched": [int(x) for x in result.keys()],
        "source": "API-Football",
        "quota": dict(_quota_state),
    }
...
SPORT_LABELS = {
    ...
    "tennis": "🎾 Tennis", "basketball": "🏀 Basketball", "american-football": "🏈 NFL",
    "baseball": "⚾ Baseball", "hockey": "🏒 Hockey", "golf": "⛳ Golf",
    "motorsport": "🏎 Motorsport", "mma": "🥊 MMA",
}
...
def _apisports_event(g: dict, sport: str):
    ...
    return {
        ...
        "league": (g.get("league") or {}).get("name", ""), "date": fixture.get("date") or g.get("date"),
...
async def _espn_scores(sport: str, date: str | None = None, league: str | None = None):
    ...
    out=[]
    for lg in leagues:
        # Cricket has several active series; the generic scoreboard is supported
        # by current ESPN feeds. For other sports the league slug is standard.
        ...
        try:
            ...
            out.extend(_espn_event(e, sport, lg) for e in (data.get("events") or []))
        except Exception as exc:
...
async def _sports_scores(sport: str, date: str | None = None, live: bool = False, league: str | None = None):
    ...
    if not out and not live:
        ...
        out=await _espn_scores(sport, date, league)
        out=[x for x in out if x.get("statusShort") == "in"] or out
...
async def sports_config():
    return {"sports": SPORT_LABELS, "providers": {
        "football":"API-Football", "rugby":"API-Sports" if SPORTS_API_KEYS.get("rugby") else "ESPN fallback",
        "basketball":"API-Sports" if SPORTS_API_KEYS.get("basketball") else "ESPN fallback",
        "cricket":"ESPN", "tennis":"ESPN", "news":"Google News RSS headlines"
    }}
...
async def sports_match_detail(sport: str, match_id: str, league: str = Query("")):
    ...
    sport=sport.lower().strip()
    if sport == "football":
        fd=await _afoot_get("/fixtures", {"id": int(match_id)})
        f=(fd.get("response") or [{}])[0]
        stats=await fixture_stats(int(match_id))
        lineups=await fixture_lineups(int(match_id))
        return {"sport":"football","match":f,"statistics":stats.get("statistics",[]),"teams":lineups.get("teams",[]),"source":"API-Football"}
    ...
    return {"sport":sport,"match":{"id":match_id,"league":(summary.get("header",{}).get("league") or {}).get("name") or league,"date":(summary.get("header",{}).get("competitions") or [{}])[0].get("date"),"home":(home.get("team") or home.get("athlete") or {}).get("displayName") or home.get("displayName") or "Home","away":(away.get("team") or away.get("athlete") or {}).get("displayName") or away.get("displayName") or "Away","homeScore":home.get("score"),"awayScore":away.get("score"),"status":(event.get("status") or {}).get("type",{}).get("detail") or "Scheduled"},"statistics":summary.get("boxscore") or summary.get("leaders") or [],"teams":[],"participants":participants,"summary":summary,"source":"ESPN"}
...
async def sports_news(sport: str = Query("all"), q: str = Query(""), limit: int = Query(30, ge=1, le=50)):
    terms={
        "all":"South Africa sport football rugby cricket tennis basketball",
        "football":"South Africa football Premier League PSL transfer injury site:goal.com OR site:metro.co.uk OR site:espn.com OR site:soccerladuma.co.za",
...
async def sports_transfers(
    ...
    if sport == "football":
        ...
        for item in data.get("response") or []:
            ...
            for tr in transfers:
                rows.append({"sport":"football","player":player.get("name"),"date":tr.get("date"),"type":tr.get("type"),
...
# ---------------------------------------------------------------------------
# PHASE 5/6 — ORGANIC GROWTH, CONTENT, RETENTION & MONETISATION TELEMETRY
# ---------------------------------------------------------------------------
...
async def _seo_dynamic_rows() -> list[dict]:
    ...
    try:
        ...
        log.warning("Dynamic SEO sitemap fixture expansion failed: %s", exc)
    return rows

@app.get("/seo/sitemap.xml", response_class=Response)
...
# ---------------------------------------------------------------------------
# SERVER-RENDERED SEO PAGES — Phase 7
# ---------------------------------------------------------------------------
...
def _ssr_match_section(match: dict, path: str) -> tuple[str,str,str]:
    ...
    body=(f'<article class="kasiscore-seo-page" data-seo-page="match"><nav aria-label="Breadcrumb"><a href="/">KasiScore Sports Intelligence</a> / Match / {_ssr_text(home)} vs {_ssr_text(away)}</nav>'
          f'<h1>{_ssr_text(home)} vs {_ssr_text(away)} — Prediction, Odds &amp; Match Intelligence</h1><p class="seo-lead">{_ssr_text(desc)}</p>'
          f'<section class="seo-grid"><div><strong>Kickoff</strong><span>{_ssr_text(kickoff)}</span></div><div><strong>Competition</strong><span>{_ssr_text(league)}</span></div>'
...
async def _ssr_team_section(slug: str, path: str) -> tuple[str,str,str]:
    ...
    body=(f'<article class="kasiscore-seo-page" data-seo-page="team"><nav aria-label="Breadcrumb"><a href="/">KasiScore Sports Intelligence</a> / Team / {_ssr_text(team_name)}</nav>'
          f'<h1>{_ssr_text(team_name)} Team Intelligence</h1><p class="seo-lead">{_ssr_text(desc)}</p><section class="seo-grid">'
...
async def _ssr_player_section(slug: str, path: str) -> tuple[str,str,str]:
    ...
    body=(f'<article class="kasiscore-seo-page" data-seo-page="player"><nav aria-label="Breadcrumb"><a href="/">KasiScore Sports Intelligence</a> / Player / {_ssr_text(title_name)}</nav>'
          f'<h1>{_ssr_text(title_name)} — Player Intelligence</h1><p class="seo-lead">{_ssr_text(desc)}</p><section class="seo-grid"><div><strong>Team</strong><span>{_ssr_text(team,"Unavailable")}</span></div><div><strong>Statistics</strong><span>{_ssr_text(stats)}</span></div></section>'
...
async def _ssr_generic_section(path: str) -> tuple[str,str,str]:
    ...
    body=f'<article class="kasiscore-seo-page" data-seo-page="hub"><nav aria-label="Breadcrumb"><a href="/">KasiScore Sports Intelligence</a> / {_ssr_text(title)}</nav><h1>{_ssr_text(title)}</h1><p class="seo-lead">{_ssr_text(desc)}</p><p>KasiScore combines live sports data, bookmaker market information and predictive intelligence. The interactive application hydrates on this same URL for live updates and deeper analysis.</p><p><a href="/today">Today</a> · <a href="/weekend">Weekend</a> · <a href="/football">Football</a></p></article>'
...
async def _render_seo_document(path: str) -> str:
    ...
    css='<style id="kasiscore-ssr-css">.kasiscore-seo-page{max-width:1180px;margin:0 auto 18px;padding:18px;border:1px solid var(--line);border-radius:15px;background:var(--card-bg);color:var(--text)}.kasiscore-seo-page h1{font-size:clamp(24px,4vw,38px);margin:10px 0}.kasiscore-seo-page h2{font-size:18px;margin:18px 0 8px}.kasiscore-seo-page a{color:var(--accent)}.seo-lead{font-size:15px;color:var(--muted);max-width:900px}.seo-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:9px;margin:14px 0}.seo-grid div{padding:11px;border:1px solid var(--line);border-radius:10px;background:var(--panel2)}.seo-grid strong,.seo-grid span{display:block}.seo-grid strong{font-size:10px;text-transform:uppercase;color:var(--muted)}.seo-grid span{font-weight:800;margin-top:4px}@media(max-width:700px){.seo-grid{grid-template-columns:1fr 1fr}}@media(max-width:440px){.seo-grid{grid-template-columns:1fr}}</style>'
...
async def weekend_intelligence():
    ...
    featured = [c for c in COMPETITION_REGISTRY if c.get("featured") and c.get("seo")]
    return {
