...
ODDSPAPI_MIN_REQUEST_INTERVAL = float(os.getenv("ODDSPAPI_MIN_REQUEST_INTERVAL", "0.85"))
...
async def team_profile(team: str = Query(...), season: int = Query(0, ge=0)):
    ...
    async def safe(path,params):
        try:return await _afoot_get_deadline(path,params,deadline=1.35)
        except Exception as exc:
...
async def tip_of_day():
    ...
    return {"state":state,"tips":tips,"count":len(tips),"originalDateRequired":state=="previous",
...
def _oddspapi_multisport_event(row: dict, sport: str) -> dict:
    ...
    return {
        "id":fid,"sport":sport,
        "league":tournament.get("tournamentName") or tournament.get("name") or sport.title(),
        "leagueSlug":str(tournament.get("tournamentId") or tournament.get("id") or ""),
        "country":tournament.get("categoryName") or tournament.get("country") or home.get("country") or "",
        ...
        "status":status.get("statusName") or status.get("name") or ("Live" if status.get("live") else "Scheduled"),
        "statusId":status.get("statusId") if status.get("statusId") is not None else status.get("id"),
        "statusShort":"in" if status.get("live") else str(status.get("statusId") if status.get("statusId") is not None else (status.get("id") if status.get("id") is not None else "pre")),
...
async def _oddspapi_participant_catalog(sport:str, ids:list)->dict:
    ...
    for v in ids:
        try:
            if v is not None and str(v).strip(): clean.append(str(v).strip())
        except Exception: pass
    clean=list(dict.fromkeys(clean))
    if not clean:return {}
    sid={"rugby":ODDSPAPI_SPORT_RUGBY,"cricket":ODDSPAPI_SPORT_CRICKET}.get(sport)
    out={}
    # Docs support participantIds lookup. Chunk to keep URLs bounded.
    for n in range(0,len(clean),80):
        chunk=clean[n:n+80]
        try:
            d=await _oddspapi_get(f"{ODDSPAPI_REST_LANGUAGE}/participants",
                                  {"participantIds":",".join(chunk),"sportId":sid})
            for z in _oddspapi_rows(d):
                pid=str(z.get("participantId") or z.get("id") or "")
                if not pid:continue
                country=z.get("country") or {}
                out[pid]={
                    "name":z.get("participantName") or z.get("name") or z.get("shortName") or "",
                    "logo":z.get("logo") or z.get("image") or z.get("badge") or z.get("logoUrl") or "",
                    "country":country.get("countryName") if isinstance(country,dict) else (country or z.get("countryName") or ""),
                    "flag":(country.get("flag") or country.get("flagUrl")) if isinstance(country,dict) else (z.get("flag") or z.get("flagUrl") or "")
                }
        except Exception as exc:
            log.warning("OddsPapi %s participant lookup failed: %s",sport,exc)
    return out
...
async def _oddspapi_sport_fixtures(sport: str, date: str | None=None, live: bool=False) -> list[dict]:
    ...
    if date:
        ...
        now=datetime.now(timezone.utc)
        start,end=(now-timedelta(hours=12),now+timedelta(hours=36))
    params={"sportId":sid,"startTimeFrom":int(start.timestamp()),"startTimeTo":int(end.timestamp())}
    try:
        data=await _oddspapi_get(f"{ODDSPAPI_REST_LANGUAGE}/fixtures",params)
    except RuntimeError as exc:
        if "HTTP 401" not in str(exc) and "HTTP 403" not in str(exc):raise
        data=await _oddspapi_get("fixtures",{
            "sportId":sid,
            "from":start.isoformat().replace("+00:00","Z"),
            "to":end.isoformat().replace("+00:00","Z"),
    ...
    rows=[_oddspapi_multisport_event(x,sport) for x in _oddspapi_rows(data)]
    rows=await _oddspapi_enrich_participants(rows,sport)
    if live: rows=[x for x in rows if str(x.get("statusShort"))=="in" or str(x.get("status")).lower()=="live"]
    return rows
...
async def _oddspapi_sport_range(sport:str,start:datetime,end:datetime,status_id:int|None=None)->list[dict]:
    ...
    if not sid or not ODDSPAPI_ENABLED or not ODDSPAPI_API_KEY:return []
    params={"sportId":sid,"startTimeFrom":int(start.timestamp()),"startTimeTo":int(end.timestamp())}
    if status_id is not None:params["statusId"]=status_id
    try:
        data=await _oddspapi_get(f"{ODDSPAPI_REST_LANGUAGE}/fixtures",params)
    except RuntimeError as exc:
        if "HTTP 401" not in str(exc) and "HTTP 403" not in str(exc):raise
        q={"sportId":sid,"from":start.isoformat().replace("+00:00","Z"),"to":end.isoformat().replace("+00:00","Z"),"language":"en"}
        data=await _oddspapi_get("fixtures",q,base=ODDSPAPI_V4_FALLBACK_BASE)
    rows=[_oddspapi_multisport_event(x,sport) for x in _oddspapi_rows(data)]
    return await _oddspapi_enrich_participants(rows,sport)
