...
async def _enrich_news_card(item: dict) -> dict:
    ...
    try:
        ...
        image=(image or "").replace("\\/","/").replace("&amp;","&").strip()
        if image.startswith("//"):image="https:"+image
        elif image.startswith("/"):
            p=urllib.parse.urlparse(final);image=f"{p.scheme}://{p.netloc}{image}"
        if _generic_news_image(image):image=""
        payload={
            "image":image or ("" if _is_generic_news_image(out.get("image","")) else out.get("image","")),
            ...
            "imageAvailable":bool(image or (out.get("image") and not _is_generic_news_image(out.get("image",""))))
        }
...
async def sports_news(sport: str = Query("all"), q: str = Query(""), limit: int = Query(30, ge=1, le=50)):
    ...
    # Let each publisher enrichment complete independently. The individual HTTP
    # requests already have bounded timeouts; a short global timeout previously
    # discarded every enriched image when one or more publishers were slow.
