...
async def _oddspapi_football_fixtures(refresh: bool = False) -> list[dict]:
    ...
    data = await _oddspapi_get(f"{ODDSPAPI_REST_LANGUAGE}/fixtures/odds", {
...
async def oddspapi_diagnostics():
    ...
    fixtures = []
    try:
        fd = await _oddspapi_get(f"/{ODDSPAPI_REST_LANGUAGE}/fixtures/odds", {
            "sportId": ODDSPAPI_SPORT_FOOTBALL,
            "from": start,
            "to": end,
            "hasOdds": "true",
    ...
    if sample_fixture_id:
        try:
            od = await _oddspapi_get(f"/{ODDSPAPI_REST_LANGUAGE}/fixtures/odds", {
...
async def oddspapi_health():
    return {
        ...
        "mappedApiFootballFixtures": len(_oddspapi_afoot_map),
        "strategy": "API-Football primary -> OddsPAPI v5 REST fallback -> cache -> prediction engine",
        "restBase": ODDSPAPI_REST_BASE,
