...
async def oddspapi_diagnostics():
    ...
    # v44: warm the existing API-Football fixture loader whenever the cache
    # contains no FUTURE fixtures. A stale/past fixture must not prevent the
    # diagnostic from loading current upcoming matches.
    mapping_warmed = False
    mapping_warm_error = None

    def _has_future_afoot(rows: list[dict]) -> bool:
    ...
    if needs_mapping_warm and AFOOT_API_KEY:
        try:
    ...
    report["mapping"]["cacheWarmSucceeded"] = bool(mapping_warmed and _has_future_afoot(candidates))
