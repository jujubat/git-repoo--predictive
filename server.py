...
# used when the primary API-Football request misses the five-second deadline
# or returns no usable bookmaker 1X2 odds.
...
async def fixtures_with_odds(
    ...
    try:
        ...
        if ODDS_API_ENABLED and ODDS_API_KEY:
            log.warning("Primary fixture/odds provider exceeded %.1fs or failed (%s); using The Odds API fallback", ODDS_API_TIMEOUT, primary_exc)
            try:
                return await _odds_api_fallback_fixtures(league)
    ...
    if refresh or not real_by_id:
        try:
            ...
            fetched = await asyncio.wait_for(
                _fetch_prematch_odds_by_dates(dates, league_ids=lids),
                timeout=ODDS_API_TIMEOUT,
            )
