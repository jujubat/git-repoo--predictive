...
# Kasi Sports News v212 editorial retention UX
app = FastAPI(
...
# ---------------------------------------------------------------------------
# Kasi Sports SQL data layer (v212)
...
def _sql_team_profile(team_id:int,season:int):
    ...
    return {"team":{"id":team_id,"name":t["name"],"code":t["code"],"country":t["country"],"founded":t["founded"],"national":bool(t["national"]),"logo":t["badge"]},
            ...
            "standings":[],"teamStats":_db_load(stats[0]["stats_json"],{}) if stats else {},"injuries":[],
...
async def team_profile(team: str = Query(...), season: int = Query(0, ge=0)):
    ...
    if raw.isdigit() and SPORTS_DB_ENABLED:
        ...
        if sql_profile:
            ...
            return sql_profile
    cache_key=f"team-profile-v212:{raw.lower()}:{season}"
    ...
    _team_profile_page_cache[f"team-profile-v212:{team_id}:{season}"]=result
    return result
...
async def _render_seo_document(path: str) -> str:
    ...
    # v212: SEO metadata/schema stay in <head>; the old visible SSR article is removed.
    _SSR_CACHE[key]=html
    return html
