...
async def fixture_odds_detail(fixture_id: int):
    """All useful bookmaker markets for a match, not only 1X2."""
    ...
    for item in data.get("response") or []:
        ...
        for bet in item.get("bets") or []:
            ...
            for v in bet.get("values") or []:
                values.append({"value": v.get("value"), "odd": v.get("odd")})
            if values:
    ...
    return {"fixtureId": fixture_id, "selected": selected, "bookmakers": bookmakers[:12],
            "source":"API-Football","updatedAt":datetime.now(timezone.utc).isoformat()}


@app.get("/news/home")
async def home_news(limit: int = Query(24, ge=6, le=60)):
    """Publisher-attributed headline feed for the KasiScore home page.

    Headlines are discovered through Google News RSS and link to the original
    publisher. KasiScore does not reproduce article bodies.
    """
    sources = [
        ("GOAL", "site:goal.com/en-za football"),
        ("Metro", "site:metro.co.uk/sport/football football"),
        ("Soccer Laduma", "site:soccerladuma.co.za football"),
    ]
    per=max(3, min(12, limit//3))
    groups=await asyncio.gather(*(_google_news(q, per) for _,q in sources), return_exceptions=True)
    items=[]
    for (label,_),group in zip(sources,groups):
        if isinstance(group,Exception): continue
        for x in group:
            y=dict(x)
            y["publisher"]=label
            items.append(y)
    ...
    return {"count":len(items[:limit]),"items":items[:limit],
            "sources":[x[0] for x in sources],"updatedAt":datetime.now(timezone.utc).isoformat()}
...
async def team_profile(team: str = Query(...), season: int = Query(0, ge=0)):
    ...
    for sr in squad_rows:
        for pl in sr.get("players") or []:
            players.append({"id":pl.get("id"),"name":pl.get("name"),"age":pl.get("age"),"number":pl.get("number"),"position":pl.get("position"),"photo":pl.get("photo")})
    def norm(items):
        out=[]
        for f in items:
            ts=f.get("teams") or {};g=f.get("goals") or {};fs=f.get("fixture") or {};
            out.append({"id":fs.get("id"),"date":fs.get("date"),"status":(fs.get("status") or {}).get("short"),"home":(ts.get("home") or {}).get("name"),"away":(ts.get("away") or {}).get("name"),"homeScore":g.get("home"),"awayScore":g.get("away"),"league":(f.get("league") or {}).get("name")})
        return out
    return {"team":p.get("team") or {"id":team_id,"name":team_name},"venue":p.get("venue") or {},"players":players,"past10":norm(recent.get("response") or []),"future":norm(future.get("response") or []),"season":season,"source":"API-Football","updatedAt":datetime.now(timezone.utc).isoformat()}

@app.get("/player/profile")
async def player_profile(player: int = Query(..., ge=1), season: int = Query(0, ge=0)):
    """Complete football player page with identity, season stats and recent matches."""
    season=season or _current_season()
    ptask=_afoot_get("/players", {"id":player,"season":season})
    recent=await _afoot_get("/fixtures", {"player":player,"season":season,"last":20})
    pd=await ptask
    rows=pd.get("response") or []
    profile=rows[0] if rows else {}
    return {"player":profile.get("player") or {},"statistics":profile.get("statistics") or [],"recentMatches":recent.get("response") or [],"season":season,"source":"API-Football","updatedAt":datetime.now(timezone.utc).isoformat()}
...
async def _sports_scores(sport: str, date: str | None = None, live: bool = False, league: str | None = None):
    ...
    if sport == "football":
        try:
            ...
            for f in (fd.get("response") or []):
                ...
                out.append({"id":str(fs.get("id") or ""),"sport":"football","league":(f.get("league") or {}).get("name") or "Football","date":fs.get("date"),"home":(ts.get("home") or {}).get("name") or "Home","away":(ts.get("away") or {}).get("name") or "Away","homeScore":goals.get("home"),"awayScore":goals.get("away"),"status":st.get("long") or st.get("short") or "Scheduled","statusShort":st.get("short") or "","elapsed":st.get("elapsed") or 0,"venue":(fs.get("venue") or {}).get("name") or "","source":"API-Football"})
    ...
    if not out and not live:
        ...
        out=await _espn_scores(sport, date, league)
        out=[x for x in out if x.get("statusShort") in ("in","live")] or out
