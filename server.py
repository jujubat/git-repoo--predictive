...
# Kasi Sports News v216 editorial retention UX
app = FastAPI(
...
def auth_login(p: AuthPayload):
    ...
    return {"token":_token(row[0]),"email":row[1],"username":row[4],"phone":row[5],"pro":bool(row[3]),"role":row[6],"admin":str(row[6] or "user").lower()=="admin"}
...
def get_current_user(authorization: Optional[str] = Header(default=None)):
    ...
    return {"id":int(row[0]),"email":str(row[1]),"pro":bool(row[2]),
            "role":str(row[3] or "user").lower(),"status":str(row[4] or "active").lower()}
