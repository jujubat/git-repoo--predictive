...
# Kasi Sports News v204 editorial retention UX
app = FastAPI(
...
async def live(
    ...
    for f in raw:
        ...
        m["homeBadge"]=m.get("homeLogo") or ""
        m["awayBadge"]=m.get("awayLogo") or ""
...
async def team_profile(team: str = Query(...), season: int = Query(0, ge=0)):
    ...
    raw=str(team).strip()
    cache_key=f"team-profile-rich:{raw.lower()}:{season}"
    cached=_team_profile_page_cache.get(cache_key)
    if cached is not None:
        return cached
    if raw.isdigit():
        team_id=int(raw); team_name=raw
    else:
    ...
    result={"team":team_obj,"venue":p.get("venue") or {},"players":players,"past10":past,"future":upcoming,"standings":standings,"league":({"id":league_hint[0],"season":league_hint[1]} if league_hint else {}),"season":season,"form":form,"goalsFor":gf,"goalsAgainst":ga,"teamStats":team_stats,"injuries":injuries,"nextOdds":next_odds,"source":"API-Football · rich cached profile","updatedAt":datetime.now(timezone.utc).isoformat()}
    _team_profile_page_cache[cache_key]=result
    _team_profile_page_cache[f"team-profile-rich:{team_id}:{season}"]=result
    return result
...
async def _render_seo_document(path: str) -> str:
    ...
    # v204: SEO metadata/schema stay in <head>; the old visible SSR article is removed.
    _SSR_CACHE[key]=html
    return html
