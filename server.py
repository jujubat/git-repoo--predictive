...
# Kasi Sports News v210 editorial retention UX
app = FastAPI(
...
async def team_profile(team: str = Query(...), season: int = Query(0, ge=0)):
    ...
    raw=str(team).strip()
    cache_key=f"team-profile-v210:{raw.lower()}:{season}"
    ...
    _team_profile_page_cache[f"team-profile-v210:{team_id}:{season}"]=result
    return result
...
async def _render_seo_document(path: str) -> str:
    ...
    # v210: SEO metadata/schema stay in <head>; the old visible SSR article is removed.
    _SSR_CACHE[key]=html
    return html
