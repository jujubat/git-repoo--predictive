...
async def fixture_live_bundle(fixture_id: int):
    """Single frontend payload for match pages. Uses cache first and degrades per component so one 429 cannot blank the whole page."""
    ...
    errors = {}
    if isinstance(fd, Exception): errors["fixture"] = str(fd)
    if isinstance(sd, Exception): errors["statistics"] = str(sd)
    if isinstance(ld, Exception): errors["lineups"] = str(ld)
    return {
        "fixture": f,
        ...
        "quota": {
            "dailyRemaining": _quota_state.get("dailyRemaining"),
            "minuteRemaining": _quota_state.get("minuteRemaining"),
            "dailyExhausted": _quota_state.get("dailyExhausted", False),
            "minuteLimited": _quota_state.get("minuteLimited", False),
        },
        "updatedAt": datetime.now(timezone.utc).isoformat(),
    }

@app.get("/match/{fixture_id}")
async def match_page_data(fixture_id: int):
    """Lightweight match envelope; unlike insights, this never fans out into form/injury/H2H requests."""
    data = await _afoot_get("/fixtures", {"id": fixture_id})
    rows = data.get("response") or []
    if not rows:
        raise HTTPException(404, detail="Fixture not found")
    return {"fixture": rows[0], "source": "API-Football", "updatedAt": datetime.now(timezone.utc).isoformat()}
