...
_PERSISTENT_TTL: dict[str, int] = {
    ...
    "/injuries":            6 * 3600,    # 3 hrs  — injury lists update on training days
    "/standings":           6 * 3600,    # 3 hrs  — standings update after matchday only
    ...
    "/odds":                   1800,     # 1 hr   — odds shift but not minute-to-minute
...
_rate_bucket = []
RATE_LIMIT_PER_MINUTE = int(os.getenv("RATE_LIMIT_PER_MINUTE", "30"))  # v185: API-Sports plan ceiling
...
# v185 provider coordinator: pace upstream requests and collapse identical concurrent calls.
# RATE_LIMIT_PER_MINUTE=30 means a minimum 2-second spacing between provider starts.
_provider_gate_lock = asyncio.Lock()
_provider_last_started = 0.0
_provider_singleflight: dict[str, asyncio.Lock] = {}

async def _provider_pace():
    global _provider_last_started
    if RATE_LIMIT_PER_MINUTE <= 0:
        return
    min_gap = max(60.0 / RATE_LIMIT_PER_MINUTE, 2.0)
    async with _provider_gate_lock:
        now = time.monotonic()
        wait = min_gap - (now - _provider_last_started)
        if wait > 0:
            await asyncio.sleep(wait)
        _provider_last_started = time.monotonic()

def _provider_flight_key(path: str, params: dict) -> str:
    return _persist_cache_key(path, params)
...
async def _afoot_get(path: str, params: dict, force_fresh: bool = False) -> dict:
    ...
    flight_key = _provider_flight_key(path, params)
    lock = _provider_singleflight.setdefault(flight_key, asyncio.Lock())
    async with lock:
        ...
        for attempt in range(3):
            ...
            try:
                ...
                if response.status_code == 429:
                    ...
                    try: delay = max(2.0, float(retry_after)) if retry_after else 10.0
                    except Exception: delay = 10.0
                    last_exc = HTTPException(429, detail={"code":"API_FOOTBALL_MINUTE_LIMIT","message":"API-Football per-minute rate limit reached."})
                    await asyncio.sleep(min(delay, 30.0))
                    continue
                if response.status_code == 401:
                    raise HTTPException(502, detail={"code":"API_FOOTBALL_AUTH","message":"API-Football authentication failed.","api_response":data})
                if response.status_code == 403:
                    raise HTTPException(502, detail={"code":"API_FOOTBALL_FORBIDDEN","message":"API-Football rejected the request (403 Forbidden).","api_response":data})
                if response.status_code >= 500:
                    last_exc = HTTPException(502, detail={"code":"API_FOOTBALL_SERVER_ERROR","message":f"API-Football server error: HTTP {response.status_code}"})
                    continue
                if response.status_code != 200:
                    raise HTTPException(502, detail={"code":"API_FOOTBALL_HTTP_ERROR","message":f"API-Football HTTP {response.status_code}","api_response":data})
                errors = data.get("errors")
                if errors and not (isinstance(errors, list) and not errors):
                    message = "; ".join(f"{k}: {v}" for k,v in errors.items()) if isinstance(errors,dict) else str(errors)
                    # API-Sports sometimes returns rate-limit errors in a HTTP 200 body. Back off.
                    if "rate" in message.lower() and "limit" in message.lower():
                        last_exc = HTTPException(429, detail={"code":"API_FOOTBALL_MINUTE_LIMIT","message":message})
                        await asyncio.sleep(10.0)
                        continue
                    raise HTTPException(502, detail={"code":"API_FOOTBALL_ERROR","message":message,"dailyRemaining":_quota_state.get("dailyRemaining"),"minuteRemaining":_quota_state.get("minuteRemaining")})
                _persistent_cache_set(path, params, data)
                return data
            except (httpx.NetworkError, httpx.TimeoutException) as exc:
                last_exc = exc
        raise HTTPException(502, detail={"code":"API_FOOTBALL_UNREACHABLE","message":f"API-Football unavailable after retries: {last_exc}"})
...
async def ai_predictions(
    ...
    for match in fixtures_data.get("matches", []):
        ...
        # v185: do not trigger standings/team-form provider calls during dashboard prediction startup.
        # Prediction uses fixture + bookmaker data immediately; deep enrichment remains available on detail/on-demand routes.
...
# KasiScore v185: 3-minute live standard, 30/min provider pacing, single-flight request coordination.
