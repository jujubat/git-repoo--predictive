...
# KasiScore v194 editorial retention UX
app = FastAPI(
...
async def _enrich_news_card(item: dict) -> dict:
    """Fetch image/summary metadata for a visible news card and cache it."""
    out=dict(item or {})
    if _generic_news_image(out.get("image")):
        out["image"]=""
    if out.get("image") and out.get("summary"):
        return out
    url=str(out.get("link") or "").strip()
    if not url:return out
    key=f"news-enriched:{url}"
    cached=NEWS_CACHE.get(key)
    if cached:
        merged=dict(out);merged.update(cached);return merged
    try:
        c=await _sports_client()
        r=await c.get(url,follow_redirects=True,timeout=httpx.Timeout(5,connect=3),headers={"User-Agent":"Mozilla/5.0 KasiScore/193"})
        text=r.text[:1_800_000]
        def meta(*names):
            for prop in names:
                for pat in (
        ...
        if not image:
            m=re.search(r'"image"\s*:\s*(?:\[\s*)?(?:"([^"]+)"|\{\s*"url"\s*:\s*"([^"]+)")',text,re.I)
            if m:image=unescape(m.group(1) or m.group(2) or "")
        if _generic_news_image(image): image=""
        payload={"image":image or out.get("image",""),"summary":summary or out.get("summary",""),"resolvedUrl":str(r.url)}
        NEWS_CACHE[key]=payload
        out.update(payload)
    except Exception as exc:
        log.debug("News image enrichment failed %s: %s",url,exc)
    return out
...
async def sports_news_article(url: str = Query(...)):
    ...
    allowed=("news.google.com","goal.com","www.goal.com","metro.co.uk","www.metro.co.uk")
    ...
    try:
        ...
        if not ("goal.com" in final_host or "metro.co.uk" in final_host or "news.google.com" in final_host):
