...
async def _oddspapi_football_fixtures(refresh: bool = False) -> list[dict]:
    ...
    # v5 /fixtures uses epoch-second startTimeFrom/startTimeTo.  Keep the legacy
    # v4 request only as an entitlement fallback for older keys.
    try:
        data = await _oddspapi_get(f"{ODDSPAPI_REST_LANGUAGE}/fixtures", {
            "sportId": ODDSPAPI_SPORT_FOOTBALL,
            "startTimeFrom": int((now - timedelta(hours=6)).timestamp()),
            "startTimeTo": int((now + timedelta(hours=36)).timestamp()),
