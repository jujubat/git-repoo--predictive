...
ODDSPAPI_REST_BASE = os.getenv("ODDSPAPI_REST_BASE", "https://api.oddspapi.io/v4").rstrip("/")
...
async def _oddspapi_get(path: str, params: Optional[dict] = None) -> Any:
    ...
    async with httpx.AsyncClient(timeout=12.0) as client:
        response = await client.get(f"{ODDSPAPI_REST_BASE}/{path.lstrip('/')}", params=query)
...
def _oddspapi_1x2(snapshot: dict) -> dict:
    ...
    Football Full Time Result:
      market 101; outcomes 101=home, 102=draw, 103=away.
    Prefer Bet365, then Betway, while accepting either configured bookmaker.
    """
    books = (snapshot or {}).get("bookmakerOdds") or {}
    ordered = list(ODDSPAPI_BOOKMAKERS) + [b for b in books if b not in ODDSPAPI_BOOKMAKERS]
    for bookmaker in ordered:
        board = books.get(bookmaker) or {}
        market = (board.get("markets") or {}).get("101") or (board.get("markets") or {}).get(101)
        if not market:
            continue
        ...
        for oid, label in (("101","homeWin"),("102","draw"),("103","awayWin")):
            ...
            player = (outcome.get("players") or {}).get("0") or (outcome.get("players") or {}).get(0) or {}
            price = player.get("price")
            if not player or player.get("active") is False or not isinstance(price, (int,float)) or price <= 1:
                active = False
                break
            prices[label] = float(price)
        if active and len(prices) == 3:
            return {
                **prices,
                "over05": 0, "over15": 0, "over25": 0, "btts": 0,
                "firstHalfHome": 0, "scoreFirst": 0, "expectedGoalscorer": 0,
                "_bookmaker": bookmaker,
                "_oddspapiFixtureId": snapshot.get("fixtureId"),
            }
    return {}


def _oddspapi_fixture_matches(afoot: dict, op: dict) -> bool:
    ah = _normalise_team_name(afoot.get("home") or afoot.get("homeTeam"))
    aa = _normalise_team_name(afoot.get("away") or afoot.get("awayTeam"))
    oh = _normalise_team_name(op.get("participant1Name"))
    oa = _normalise_team_name(op.get("participant2Name"))
    ...
    if not (exact or fuzzy):
        return False
    at = _parse_provider_time(afoot.get("datetime") or afoot.get("kickoff") or afoot.get("date"))
    ot = _parse_provider_time(op.get("startTime"))
    return not (at and ot and abs((at - ot).total_seconds()) > 3 * 3600)


async def _oddspapi_football_fixtures(refresh: bool = False) -> list[dict]:
    key = "football-window"
    if not refresh:
    ...
    # OddsPAPI allows sportId + from/to. Keep this window below 48 hours.
    data = await _oddspapi_get("fixtures", {
        "sportId": ODDSPAPI_SPORT_FOOTBALL,
        "from": (now - timedelta(hours=6)).isoformat().replace("+00:00","Z"),
        "to": (now + timedelta(hours=36)).isoformat().replace("+00:00","Z"),
        "hasOdds": "true",
        "bookmakers": ",".join(ODDSPAPI_BOOKMAKERS),
        "language": "en",
    })
    fixtures = data if isinstance(data, list) else []
    _oddspapi_fixture_cache[key] = fixtures
    return fixtures
...
async def _oddspapi_resolve_fixture(afoot_fixture_id: Any, meta: dict, refresh: bool = False) -> Optional[str]:
    ...
    for fixture in fixtures:
        if _oddspapi_fixture_matches(meta, fixture):
            opid = str(fixture.get("fixtureId"))
...
async def _supplement_odds_from_oddspapi(matches: list[dict], refresh: bool = False) -> dict:
    ...
    meta["checked"] = True
    try:
        # One cached fixture-window request, then odds calls only for API-Football gaps.
        await _oddspapi_football_fixtures(refresh=refresh)
...
async def oddspapi_diagnostics():
    ...
    report = {
        ...
        "odds": {"requestOk": False, "count": 0, "bookmakersSeen": [], "selectedSeen": [], "sample": []},
    ...
    try:
        ...
        for f in fixtures[:5]:
            report["fixtures"]["sample"].append({
                ...
                "participant1": f.get("participant1Name") or f.get("homeTeam") or f.get("home"),
                "participant2": f.get("participant2Name") or f.get("awayTeam") or f.get("away"),
    ...
    try:
        ...
        for row in odds_rows:
            bo = row.get("bookmakerOdds") or row.get("bookmakers") or {}
            if isinstance(bo, dict):
                iterable = bo.keys()
            elif isinstance(bo, list):
                iterable = [(x.get("slug") or x.get("name") or x.get("bookmaker")) for x in bo if isinstance(x, dict)]
            else:
                iterable = []
            for name in iterable:
                ...
                seen.add(str(name))
                if any(b.lower().replace(" ", "") in slug or slug in b.lower().replace(" ", "") for b in ODDSPAPI_BOOKMAKERS):
        ...
        for row in odds_rows[:5]:
            extracted = _oddspapi_extract_1x2(row)
            report["odds"]["sample"].append({
                "fixtureId": row.get("fixtureId") or row.get("id"),
                "has1X2": bool(extracted),
                "oneXTwo": extracted if extracted else None,
            })
    except Exception as exc:
        report["errors"].append(f"odds request failed: {type(exc).__name__}: {exc}")

    # Inspect only already-cached API-Football fixtures; diagnostics must not
    # consume extra API-Football quota just to produce a report.
    candidates = []
    try:
        for key, value in list(_fixture_cache.items()):
            rows = value if isinstance(value, list) else (value.get("response", []) if isinstance(value, dict) else [])
            for item in rows:
        ...
        for af in candidates[:100]:
            ...
            if found:
                ...
                if len(samples) < 5:
                    ...
                    samples.append({
                        "apiFootballFixtureId": fx.get("id"),
                        "oddsPapiFixtureId": found.get("fixtureId") or found.get("id"),
                        "matched": True,
                    })
            elif len(samples) < 5:
                fx = af.get("fixture", {}) if isinstance(af.get("fixture"), dict) else {}
                ...
                samples.append({
                    "apiFootballFixtureId": fx.get("id"),
                    "home": (teams.get("home") or {}).get("name") if isinstance(teams.get("home"), dict) else None,
                    "away": (teams.get("away") or {}).get("name") if isinstance(teams.get("away"), dict) else None,
                    "date": fx.get("date"),
                    "matched": False,
                    "reason": "no team/time match in returned OddsPAPI fixtures",
                })
        report["mapping"]["matched"] = matched
        report["mapping"]["unmatched"] = max(0, min(len(candidates), 100) - matched)
...
async def oddspapi_health():
    return {
        ...
        "mappedApiFootballFixtures": len(_oddspapi_afoot_map),
        "strategy": "API-Football primary -> OddsPAPI v4 REST fallback + v5 resumable WebSocket -> cache -> prediction engine",
        "restBase": ODDSPAPI_REST_BASE,
...
async def _build_news_pool() -> list[dict]:
...
async def sports_news(
    ...
    auto=await _approx_video_location(request)
    visitor_country=(country.strip().upper() or str(auto.get("country") or "").upper())
    ...
    if market_query:
        ...
        market=_NEWS_POOL_CACHE.get(ck)
        if market is None:
            try:
                rows=await _google_news(market_query,30)
            except Exception:
                rows=[]
            market=[]
            for row in rows:
                x=dict(row);x["sport"]=x.get("sport") or "sport"
                x["publisher"]=x.get("source") or "Sports News";x["countries"]=_news_country_from_text(x,visitor_country)
                x.update(_news_tags(x))
                target=str(x.get("publisherUrl") or x.get("resolvedUrl") or x.get("link") or "")
                x["articleId"]=hashlib.sha1(target.encode()).hexdigest()[:16] if target else hashlib.sha1(str(x.get("title","")).encode()).hexdigest()[:16]
                x["articlePath"]=f"/news/{x['articleId']}?source={urllib.parse.quote(target,safe='')}" if target else ""
                x["originalUrl"]=target;x["description"]=x.get("summary") or x.get("description") or ""
                x["imageAvailable"]=bool(x.get("image") or x.get("ogImage") or x.get("publisherImage"))
                x["author"]=x.get("author") or ""
                market.append(x)
            _NEWS_POOL_CACHE[ck]=list(market)
...
async def sports_news_bulletin(request: Request, limit: int = Query(5,ge=1,le=8), country: str = Query("")):
    try:
        ...
        pool=await _build_news_pool()
        items=[x for x in pool if isinstance(x,dict)][:limit]
