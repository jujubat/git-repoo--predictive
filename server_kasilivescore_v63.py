...
async def sports_directory(sport:str=Query(...), q:str=Query("")):
    ...
    if sport=="football":
        try:
            ...
            else: errors.append("Enter a team or player name to search the global football provider.")
...
@app.get("/sports/{sport}/match/{match_id}/details")
async def sport_match_details(sport:str,match_id:str):
    """On-demand match details for Live/Finished Stats. No bulk preloading."""
    sp=(sport or "").strip().lower()
    mid=(match_id or "").strip()
    if sp not in {"football","rugby","cricket"} or not mid:
        raise HTTPException(404,"Match not found")
    if sp=="football":
        parts=await asyncio.gather(
            fixture_stats(int(mid)), fixture_events(int(mid)), fixture_lineups(int(mid)),
            return_exceptions=True
        )
        def ok(x): return {} if isinstance(x,Exception) else x
        return {"sport":"football","matchId":mid,"statistics":ok(parts[0]),"events":ok(parts[1]),"lineups":ok(parts[2])}
    # Multisport providers expose normalized match detail where available.
    try:
        data=await sports_match_detail(sp,mid)
        if isinstance(data,dict):
            return {"sport":sp,"matchId":mid,**data}
        return {"sport":sp,"matchId":mid,"data":data}
    except Exception as exc:
        raise HTTPException(502,f"{sp.title()} match details unavailable: {exc}")
