...
async def _oddspapi_football_fixtures(refresh: bool = False) -> list[dict]:
    ...
    data, _rest_version = await _oddspapi_v5_or_v4(f"{ODDSPAPI_REST_LANGUAGE}/fixtures/odds", "fixtures", {
...
async def oddspapi_diagnostics():
    ...
    fixtures = []
    try:
        fd, fixture_api_version = await _oddspapi_v5_or_v4(f"/{ODDSPAPI_REST_LANGUAGE}/fixtures/odds", "fixtures", {
            "sportId": ODDSPAPI_SPORT_FOOTBALL,
            "from": start,
            "to": end,
            "hasOdds": "true",
