...
async def _football_match_details_build(fid:int):
    ...
    fd,stats,lineups,events,insights,players,odds_detail=await asyncio.gather(
        bounded(_afoot_get("/fixtures",{"id":fid}),4.5),
        bounded(fixture_stats(fid),4.5),
        bounded(fixture_lineups(fid),4.5),
        bounded(fixture_events(fid),4.5),
        bounded(fixture_insights(fid),4.5),
        bounded(_afoot_get("/fixtures/players",{"fixture":fid}),4.5),
        bounded(fixture_odds_detail(fid),4.5),
        return_exceptions=True)
    provider_error=""
...
async def sport_match_details(sport:str,match_id:str,league:str=Query("")):
    ...
    if sport=="football":
        ...
        snap=_dashboard_snapshot_get(f"match:football:{fid}")
        ...
        try:
            return await asyncio.wait_for(_football_match_details_build(fid),timeout=6.0)
        except Exception as exc:
...
function intelligence(i){{i=i||{{}};const h=i.home||{{}},a=i.away||{{}},h2h=i.h2h||[];return `<div class="grid2"><div class="card"><h3>Recent form & league position</h3><p><b>${{E(h.name||'Home')}}</b> · ${{E(h.form||'—')}} · #${{E(h.leaguePosition??'—')}}</p><p><b>${{E(a.name||'Away')}}</b> · ${{E(a.form||'—')}} · #${{E(a.leaguePosition??'—')}}</p></div><div class="card"><h3>Head to head</h3>${{h2h.length?h2h.slice(0,5).map(x=>`<div class="tiny">${{E(x.home||'')}} ${{E(x.score||'')}} ${{E(x.away||'')}}</div>`).join(''):'<div class="empty">No H2H returned.</div>'}}</div></div>`}}
function odds(o){{o=o||{{}};const x=o.bookmakers||o.items||o.odds||[];return `<div class="card"><h3>Odds / prediction</h3><p class="tiny muted">Only provider-returned information is displayed.</p><pre style="white-space:pre-wrap;overflow:auto;color:#d5d9df">${{E(JSON.stringify(x,null,2))}}</pre></div>`}}
