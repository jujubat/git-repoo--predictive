...
from fastapi.responses import HTMLResponse, FileResponse, Response, Response, StreamingResponse
    ...
    try:
        ...
        if row:
            ...
            if age < row["ttl_seconds"]:
                ...
                # v52: an empty API-Football /fixtures response is not useful
                # as a persistent cache hit. Bypass it so the next request
                # reaches API-Football and can repopulate valid fixtures.
                if path == "/fixtures" and not (cached_data.get("response") or []):
                    return None
                return cached_data
    except Exception as e:
...
def _persistent_cache_set(path: str, params: dict, data: dict):
    ...
    # v52: preserve only useful fixture snapshots. Empty fixture responses
    # are treated as transient and must not overwrite/reinforce good state.
    if path == "/fixtures" and not (data.get("response") or []):
        return
...
async def lifespan(app: FastAPI):
    ...
    home_warm_task = asyncio.create_task(_build_home_football_snapshot())
...
async def export_fixtures_pdf(payload: FixtureExportPayload):
    ...
    try:
        ...
        raise HTTPException(503,"PDF export dependency is not installed")
...
async def kasi_production_security_headers(request: Request, call_next):
    ...
    response.headers.setdefault("Content-Security-Policy",
        ...
        "style-src 'self' 'unsafe-inline'; connect-src 'self' https:; font-src 'self' https: data:; object-src 'none'; base-uri 'self'; frame-ancestors 'self'")
    if request.url.path.startswith(("/auth/","/admin/")):
        response.headers["Cache-Control"]="no-store"
    return response

# v56 fast homepage football snapshot
...
async def fixture_lineups(fixture_id: int):
    ...
    try:
        ...
        rows=data.get("response") or []; teams=[]
        for row in rows:
...
async def fixture_odds_detail(fixture_id: int):
    ...
    # v27: OddsPAPI is the preferred fallback after API-Football.
    if ODDSPAPI_ENABLED and ODDSPAPI_API_KEY:
        meta = _fixture_meta_cache.get(str(fixture_id), {})
        try:
            norm, opid = await _oddspapi_odds_for_afoot(fixture_id, meta, refresh=bool(refresh))
            valid = _valid_1x2(norm)
            if valid:
                cached = {
                    "fixture_id": fixture_id, "odds": norm, "source": "OddsPAPI",
                    "oddsPapiFixtureId": opid,
                    "bookmaker": norm.get("_bookmaker"),
                    "fetchedAt": datetime.now(timezone.utc).isoformat(),
                }
                _odds_cache[cache_key] = cached
                return {**cached, "available": True, "bookmakerGatePassed": True, "fallback": True}
        except Exception as exc:
            log.warning("OddsPAPI fixture fallback failed: %s", exc)

    # Legacy fallback remains available only if separately configured.
    # v48: API-Football remains primary. If it has no usable 1X2, try the
    # already-integrated OddsPAPI resolver next (Bet365 preferred, Betway
    # fallback) before the legacy The Odds API fallback.
    meta = _fixture_meta_cache.get(str(fixture_id), {})
    if ODDSPAPI_ENABLED and ODDSPAPI_API_KEY and meta:
        try:
            op_odds, opid = await _oddspapi_odds_for_afoot(
                fixture_id, meta, refresh=bool(refresh)
            )
            if _valid_1x2(op_odds):
                result = {
                    ...
                    "fallback": True,
                    "oddsPapiFixtureId": opid,
                    "bookmaker": op_odds.get("_bookmaker"),
                    "quota": dict(_quota_state),
                }
            ...
            log.warning("OddsPAPI fixture fallback failed for %s: %s", fixture_id, exc)
...
_VIDEO_SOURCES = {
    ...
    "cricket": [
        ...
        ("England & Wales Cricket Board","@englandcricket",["GB"],["England","Wales"],[],["England Cricket"]),
    ],
}
    ...
    try:
        ...
        for entry in root.findall("atom:entry",ns)[:limit]:
            ...
            if group is not None:
                ...
                description=(group.findtext("media:description",default="",namespaces=ns) or "").strip()[:500]
...
async def sports_videos(
    ...
    # Preserve fallback diversity: ranking naturally moves from local to regional,
    # national, then general while freshness orders each relevance tier.
...
async def team_profile(team: str = Query(...), season: int = Query(0, ge=0)):
    ...
    async def safe(path,params):
        try:return await _afoot_get_deadline(path,params,deadline=4.0)
        except Exception as exc:
...
async def team_injuries(team: str = Query(...)):
    """Compatibility endpoint: injury data is disabled and makes no upstream request."""
    return {"team":team,"count":0,"players":[],"disabled":True,"message":"Injury data is currently disabled."}
...
async def tip_of_day():
    """Top two current bookmaker-qualified Kasi picks, with stored fallback and yesterday resolution."""
    today=datetime.now().strftime("%Y-%m-%d")
    yesterday=(datetime.now()-timedelta(days=1)).strftime("%Y-%m-%d")
    current=[]
    provider_error=None
    try:
        d=await ai_predictions(limit=100,refresh=0,league="ALL",type="upcoming",date_from=today,date_to=today)
    ...
    try:
        with v141_learning.connect() as conn:
            ...
            fallback=sorted([stored_item(r) for r in rows],
                            key=lambda x:float((x.get("prediction") or {}).get("confidence") or 0),reverse=True)[:2]
            yr=conn.execute("""
    ...
    tips=current[:2] if current else fallback[:2]
    state="current" if current else ("previous" if fallback else "error")
    payload={"state":state,"tips":tips,"count":len(tips),"originalDateRequired":state=="previous",
            "yesterdayDate":yesterday,"yesterday":yesterday_rows,
            "error":provider_error if state=="error" else None,
            "generatedAt":datetime.now(timezone.utc).isoformat()}
    ...
    try:
        ...
        if tips:
            ...
            row=con.execute("SELECT payload FROM dashboard_snapshots WHERE key='tip-of-day'").fetchone()
            if row:
                cached=json.loads(row[0]);cached["state"]="cached";cached["cached"]=True
                cached["refreshError"]=provider_error;payload=cached
        con.close()
...
async def finished_games(days:int=Query(3,ge=1,le=7),league:str=Query("ALL"),limit:int=Query(60,ge=1,le=100)):
    """Lightweight completed football fixtures. Detailed match data stays on-demand."""
    end=datetime.now()
    ...
    for i in range(days):
        ...
        try:
            d=await fixtures(league=league,type="today",date_from=day,date_to=day,refresh=0)
            for m in d.get("matches") or []:
                if m.get("isFinished") or str(m.get("status") or "").upper() in {"FT","AET","PEN"}:
                    rows.append(m)
        except Exception as exc:
            errors.append(f"{day}: {exc}")
    rows.sort(key=lambda x:x.get("datetime") or x.get("date") or "",reverse=True)
    return {"count":len(rows[:limit]),"matches":rows[:limit],"days":days,"errors":errors[:3]}
...
def _oddspapi_multisport_event(row: dict, sport: str) -> dict:
    ...
    return {
        ...
        "homeScore":result.get("participant1Score") if result.get("participant1Score") is not None else result.get("home"),
        "awayScore":result.get("participant2Score") if result.get("participant2Score") is not None else result.get("away"),
        "status":row.get("statusName") or status.get("statusName") or status.get("name") or ("Live" if status.get("live") else "Scheduled"),
        ...
        "statusShort":"in" if (row.get("statusId")==1 or status.get("live")) else str(row.get("statusId") if row.get("statusId") is not None else (status.get("statusId") if status.get("statusId") is not None else (status.get("id") if status.get("id") is not None else "pre"))),
...
async def _oddspapi_participant_catalog(sport:str, ids:list)->dict:
    ...
    if not clean:return {}
    sid={"rugby":ODDSPAPI_SPORT_RUGBY,"cricket":ODDSPAPI_SPORT_CRICKET}.get(sport)
    try:
...
async def _oddspapi_sport_fixtures(sport: str, date: str | None=None, live: bool=False) -> list[dict]:
    ...
    if date:
        ...
        now=datetime.now(timezone.utc);start,end=now-timedelta(hours=12),now+timedelta(hours=36)
    if live:
        now=datetime.now(timezone.utc)
    ...
    rows=await _oddspapi_sport_range(sport,start,end,None)
    return rows
...
async def _oddspapi_sport_range(sport:str,start:datetime,end:datetime,status_id:int|None=None)->list[dict]:
    ...
    # v4 requires sportId date windows to remain under 10 days. Split longer
    # history windows so Rugby/Cricket prediction history cannot be rejected.
    raw=[]
    cursor=start
    while cursor < end:
        ...
        q={"sportId":sid,
           ...
           "language":"en"}
        if status_id is not None:q["statusId"]=status_id
        d=await _oddspapi_get("fixtures",q,base=ODDSPAPI_V4_FALLBACK_BASE)
        raw.extend(_oddspapi_rows(d))
        cursor=stop
    # De-duplicate a boundary fixture if provider includes both ends.
    seen=set(); rows=[]
    for z in raw:
    ...
    # Names should already come from v4 fixtures; catalog is only a fallback.
    unresolved=[x for x in rows if x.get("home") in {"Home",""} or x.get("away") in {"Away",""}]
    if unresolved: await _oddspapi_enrich_participants(unresolved,sport)
    return rows
...
async def sport_match_details(sport:str,match_id:str,league:str=Query("")):
    ...
    if sport=="football":
        ...
        fd=await _afoot_get("/fixtures",{"id":fid}); rr=fd.get("response") or []
        f=rr[0] if rr else {}
        stats,lineups,events,insights=await asyncio.gather(fixture_stats(fid),fixture_lineups(fid),fixture_events(fid),fixture_insights(fid),return_exceptions=True)
        stats=stats if isinstance(stats,dict) else {};lineups=lineups if isinstance(lineups,dict) else {};events=events if isinstance(events,dict) else {};insights=insights if isinstance(insights,dict) else {}
        return {"sport":"football","matchId":fid,"match":f,"statistics":stats.get("statistics",[]),"teams":lineups.get("teams",[]),"events":events.get("events",[]),"insights":insights,"source":"API-Football","available":bool(f),"updatedAt":datetime.now(timezone.utc).isoformat()}
    if sport not in {"rugby","cricket"}:raise HTTPException(404,"Sport not supported")
    f=await _oddspapi_find_fixture(sport,match_id)
    if not f:return {"sport":sport,"matchId":match_id,"match":{},"statistics":[],"events":[],"available":False,"source":"OddsPapi","error":"Fixture data was not returned by the subscribed provider."}
    hr,ar=await asyncio.gather(_multi_recent(f.get("homeId"),sport,5),_multi_recent(f.get("awayId"),sport,5))
    return {"sport":sport,"matchId":f.get("id"),"match":f,
            "statistics":{"homeForm":_multi_team_form(hr,f.get("homeId")),"awayForm":_multi_team_form(ar,f.get("awayId"))},
            "events":[],"available":True,"source":"OddsPapi","updatedAt":datetime.now(timezone.utc).isoformat()}
...
async def sports_news(
    ...
    if cached is not None:
        ...
        await _refresh_news_pool_background()
        try:
            pool=await asyncio.wait_for(_build_news_pool(),timeout=NEWS_FAST_WAIT)
        except asyncio.TimeoutError:
            pool=list(_NEWS_LAST_GOOD)
...
async def sports_news_bulletin(request: Request, limit: int = Query(5,ge=1,le=8), country: str = Query("")):
    try:
...
@app.get("/sports/news/dashboard")
async def sports_news_dashboard(limit: int = Query(12, ge=1, le=30)):
    """Customer-facing qualified news: cache first; fresh cards only after article+image verification."""
    key=f"dashboard:{limit}"
    now=time.time()
    cached=_NEWS_DASH_CACHE.get(key)
    if cached and now-cached["ts"] < _NEWS_DASH_TTL:
        return {"items":cached["items"],"count":len(cached["items"]),"cached":True,"qualityChecked":True}
    try:
        # Reuse the established multi-source news pipeline, then gate what reaches customers.
        fresh=await sports_news(sport="all",limit=min(60,max(limit*3,30)),offset=0)
        raw=fresh.get("items",[]) if isinstance(fresh,dict) else []
        qualified=await _news_quality_filter(raw,limit)
        if qualified:
            _NEWS_DASH_CACHE[key]={"ts":now,"items":qualified}
            return {"items":qualified,"count":len(qualified),"cached":False,"qualityChecked":True}
    except Exception:
        pass
    # Fresh validation failed: serve last known-good cache, even if stale.
    if cached:
        return {"items":cached["items"],"count":len(cached["items"]),"cached":True,"stale":True,"qualityChecked":True}
    return {"items":[],"count":0,"cached":False,"qualityChecked":True}


# v86 — dashboard news quality/cache gate.
# Only cards with a usable publisher URL, readable headline and verified image are eligible.
# A last-good snapshot is returned if a fresh refresh cannot produce enough qualified cards.
_NEWS_DASH_CACHE = {}
_NEWS_DASH_TTL = 300

def _news_http_url(v):
    s=str(v or "").strip()
    return s if s.startswith(("http://","https://")) else ""

def _news_card_quality(item):
    if not isinstance(item,dict): return False
    title=str(item.get("title") or "").strip()
    link=_news_http_url(item.get("originalUrl") or item.get("publisherUrl") or item.get("resolvedUrl") or item.get("link") or item.get("url"))
    image=_news_http_url(item.get("image") or item.get("imageUrl") or item.get("thumbnail"))
    # Do not expose hash/internal IDs as article content.
    if len(title)<12 or re.fullmatch(r"[0-9a-fA-F]{12,}",title.replace(" ","")): return False
    if not link or not image: return False
    return True

async def _news_image_reachable(url):
    if not _news_http_url(url): return False
    try:
        async with httpx.AsyncClient(timeout=httpx.Timeout(3.0),follow_redirects=True) as c:
            r=await c.get(url,headers={"User-Agent":"Mozilla/5.0","Accept":"image/*"})
            ct=(r.headers.get("content-type") or "").lower()
            return r.status_code<400 and ct.startswith("image/") and len(r.content)>256
    except Exception:
        return False

async def _news_article_reachable(url):
    if not _news_http_url(url): return False
    try:
        async with httpx.AsyncClient(timeout=httpx.Timeout(4.0),follow_redirects=True) as c:
            r=await c.get(url,headers={"User-Agent":"Mozilla/5.0","Accept":"text/html,application/xhtml+xml"})
            ct=(r.headers.get("content-type") or "").lower()
            return r.status_code<400 and ("html" in ct or "xml" in ct) and len(r.content)>512
    except Exception:
        return False

async def _news_quality_filter(items,limit=24):
    candidates=[x for x in (items or []) if _news_card_quality(x)]
    async def check(x):
        image=x.get("image") or x.get("imageUrl") or x.get("thumbnail")
        link=x.get("originalUrl") or x.get("publisherUrl") or x.get("resolvedUrl") or x.get("link") or x.get("url")
        ok_img,ok_article=await asyncio.gather(_news_image_reachable(image),_news_article_reachable(link))
        return x if ok_img and ok_article else None
    # Bound verification work while allowing replacement articles to qualify.
    checked=await asyncio.gather(*(check(x) for x in candidates[:max(limit*3,30)]),return_exceptions=True)
    return [x for x in checked if isinstance(x,dict)][:limit]
...
async def seo_news_article(slug: str, source: str = Query("")):
    ...
    # The route slug may be a generated hash. Never expose it as article content.
    title=(meta.get("title") or "").strip()
    desc=(meta.get("summary") or meta.get("description") or "").strip()
    image=(meta.get("image") or "").strip()
    publisher=(meta.get("source") or "").strip() or urllib.parse.urlparse(publisher_url).netloc.replace("www.","") or "Sports News"
    if not title:
        # No fabricated customer-facing article. If publisher metadata cannot be resolved,
        # send the reader to the real source instead of rendering an empty/fake preview.
        return RedirectResponse(url=publisher_url,status_code=302)
    if not desc:
        desc=""
    ...
    img_html=f'<img src="{escape(image,quote=True)}" alt="{escape(title,quote=True)}" loading="eager" fetchpriority="high" onerror="this.remove()" style="width:100%;max-height:520px;object-fit:cover;border-radius:12px;margin:18px 0">' if image else ""
    article_body=(f'<main class="ks-article-page"><nav class="ks-back"><a href="/" data-kasi-back>← Back to Kasi Sports News</a><span>/ News</span></nav>{img_html}<div class="ks-category">SPORTS NEWS</div><h1>{escape(title)}</h1><div class="ks-source">Source: <strong>{escape(publisher)}</strong></div><p class="ks-lead">{escape(desc)}</p><div class="ks-original"><p>Kasi Sports News displays the publisher-permitted article details and summary. The original publisher remains the source of the complete copyrighted article.</p><a href="{escape(publisher_url,quote=True)}" target="_blank" rel="noopener noreferrer">Read Original Article</a></div></main>')
...
<style>
:root{{--bg:#05080c;--panel:#090e14;--text:#f4f7fb;--muted:#8ea6ba;--line:#22303c;--accent:#22b8d6;--news:#ff4d5f}}
*{{box-sizing:border-box}}html,body{{margin:0;background:var(--bg);color:var(--text);font-family:Inter,Arial,sans-serif}}
body{{min-height:100vh}}a{{color:inherit}}
.ks-header{{height:74px;border-bottom:1px solid var(--line);display:flex;align-items:center;padding:0 max(18px,calc((100vw - 1120px)/2));gap:12px;background:#070b10;position:sticky;top:0;z-index:5}}
.ks-logo{{width:44px;height:44px;border:1px solid #26313d;border-radius:11px;background:#0b1016;display:flex;align-items:center;justify-content:center;font-weight:950;font-size:21px;letter-spacing:-2px}}
.ks-logo .k{{color:#f4f7fb}}.ks-logo .s{{color:var(--accent)}}.ks-logo .n{{color:var(--news)}}
.ks-brand{{font-weight:900;font-size:20px}}.ks-brand span{{color:var(--accent)}}.ks-home{{margin-left:auto;text-decoration:none;border:1px solid var(--line);padding:9px 13px;border-radius:8px;font-size:13px}}
...
.ks-back{{display:flex;gap:8px;color:var(--muted);font-size:13px;margin-bottom:24px}}.ks-back a{{color:var(--accent);text-decoration:none}}
.ks-article-page img{{width:100%;max-height:520px;object-fit:cover;border-radius:10px;margin:0 0 24px}}
.ks-category{{display:inline-block;color:var(--accent);font-weight:900;font-size:12px;letter-spacing:.08em;margin-bottom:10px}}
h1{{font-size:clamp(32px,6vw,64px);line-height:1.03;margin:0 0 16px;letter-spacing:-.025em}}
.ks-source{{color:var(--muted);font-size:13px;margin-bottom:26px}}
.ks-lead{{font-size:clamp(17px,2.2vw,20px);line-height:1.75;max-width:800px}}
.ks-original{{margin-top:34px;padding-top:22px;border-top:1px solid var(--line);color:var(--muted);font-size:13px;line-height:1.6}}
.ks-original a{{display:inline-block;margin-top:10px;background:var(--accent);color:#041017;text-decoration:none;font-weight:900;padding:11px 15px;border-radius:8px}}
@media(max-width:600px){{.ks-header{{height:64px}}.ks-logo{{width:38px;height:38px}}.ks-brand{{font-size:17px}}.ks-article-page{{margin-top:18px}}}}
</style>
<script type="application/ld+json">{json.dumps(schema,ensure_ascii=False)}</script>
</head>
<body>
<header class="ks-header"><div class="ks-logo"><span class="k">K</span><span class="s">S</span><span class="n">n</span></div><div class="ks-brand">Kasi <span>Sports News</span></div><a class="ks-home" href="/" data-kasi-back>Dashboard</a></header>
{article_body}
<script>
(function(){{
  function returnToKasi(e){{
    if(e)e.preventDefault();
    // history.back() returns to the exact previous dashboard state and normally restores
    // it from the browser's back/forward cache without reloading the dashboard.
    if(history.length>1){{ history.back(); return; }}
    location.replace('/');
  }}
  document.querySelectorAll('[data-kasi-back], .ks-back a').forEach(function(a){{
    a.addEventListener('click',returnToKasi);
  }});
}})();
</script>
</body></html>"""
    return HTMLResponse(article_html,headers={"Cache-Control":"public, max-age=300","X-Robots-Tag":"index, follow"})
...
async def seo_sitemap():
    rows = await _seo_dynamic_rows()
    origin = os.getenv("PUBLIC_SITE_URL", "https://kasilivescore.com").rstrip("/")
    body = ['<?xml version="1.0" encoding="UTF-8"?>','<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">']
...
async def _ssr_team_section(slug: str, path: str) -> tuple[str,str,str]:
    ...
    try:
        ...
        team_name=slug.replace("-"," ").strip().title(); matches=[]; form=[]; players=[]
...
<title>{title} | Kasi Sports News</title>
<style>
:root{{--bg:#000;--card:#101214;--line:#292d32;--text:#fff;--muted:#aab2bd;--accent:#00a9ce}}
...
.back{{padding:10px 14px;border:1px solid var(--line);border-radius:9px;text-decoration:none;font-weight:800}}
...
.grid{{display:grid;grid-template-columns:1fr 1fr;gap:12px}}.row{{display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;padding:9px 0;border-bottom:1px solid var(--line)}}.muted{{color:var(--muted)}}
.person{{display:flex;gap:10px;align-items:center;padding:8px 0;border-bottom:1px solid var(--line);cursor:pointer}}
@media(max-width:700px){{.grid,.hero{{grid-template-columns:1fr}}.score{{font-size:28px}}}}
</style></head><body class="ks-standalone-page"><main><div class="top"><a class="back" href="/">← Back to Home</a><b>Kasi Sports News</b></div>
<div id="page"><div class="card">Loading {title.lower()}…</div></div></main>
<script>
const KIND={json.dumps(kind)},ID={eid},SPORT={sp},LEAGUE={lg},page=document.getElementById('page');
const E=v=>String(v??'').replace(/[&<>"']/g,c=>({{'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}}[c]));
async function J(u){{const c=new AbortController(),t=setTimeout(()=>c.abort(),5000);try{{const r=await fetch(u,{{signal:c.signal,headers:{{Accept:'application/json'}}}});if(!r.ok)throw Error('HTTP '+r.status);return await r.json()}}finally{{clearTimeout(t)}}}}
...
async function match(){{
 try{{const d=await J('/sports/'+encodeURIComponent(SPORT)+'/match/'+encodeURIComponent(ID)+(LEAGUE?'?league='+encodeURIComponent(LEAGUE):'')),m=d.match||{{}};
  ...
  }}else{{const x=m.fixture||m;page.innerHTML=`<div class="card"><h1>${{E(x.home||x.homeTeam||m.home||'Home')}} vs ${{E(x.away||x.awayTeam||m.away||'Away')}}</h1><p>${{E(x.league||m.league||SPORT)}}</p><p class="score">${{E(x.homeScore??'—')}} - ${{E(x.awayScore??'—')}}</p><p>${{E(x.status||'')}}</p></div><div class="card"><h2>Match data</h2><pre style="white-space:pre-wrap">${{E(JSON.stringify(d.statistics||d.summary||{{}},null,2))}}</pre></div>`}}
 }}catch(e){{page.innerHTML=`<div class="card"><h2>Match data unavailable</h2><p>${{E(e.message)}}</p></div>`}}
}}
...
async function team(){{
 try{{const id=await resolve('team',ID),d=await J('/team/profile?team='+encodeURIComponent(id)),t=d.team||{{}},players=d.players||[];
 page.innerHTML=`<div class="card"><div class="hero"><div>${{t.logo?`<img class="avatar" src="${{E(t.logo)}}">`:''}}</div><div><h1>${{E(t.name||'Team')}}</h1><p class="muted">${{E(t.country||'')}} · ${{E((d.venue||{{}}).name||'')}}</p></div><div></div></div></div><div class="grid"><div class="card"><h2>Squad</h2>${{players.map(x=>`<div class="person" onclick="location.href='/players/${{encodeURIComponent(x.id)}}'">${{x.photo?`<img class="avatar" src="${{E(x.photo)}}">`:''}}<div><b>${{E(x.name)}}</b><div class="muted">${{E(x.position||'')}} · #${{E(x.number??'—')}}</div></div></div>`).join('')||'<div class="muted">No squad returned.</div>'}}</div><div class="card"><h2>Team data</h2><p>Form: ${{E((d.form||[]).join(' ')||'—')}}</p><p>Goals for: ${{E(d.goalsFor??'—')}}</p><p>Goals against: ${{E(d.goalsAgainst??'—')}}</p></div></div>`;
 }}catch(e){{page.innerHTML=`<div class="card"><h2>Team data unavailable</h2><p>${{E(e.message)}}</p></div>`}}
}}
async function player(){{
 try{{const id=await resolve('player',ID),d=await J('/player/profile?player='+encodeURIComponent(id)),x=d.player||d.profile?.player||{{}},stats=d.statistics||d.teams||[];
 page.innerHTML=`<div class="card"><div class="hero"><div>${{x.photo?`<img class="avatar" src="${{E(x.photo)}}">`:''}}</div><div><h1>${{E(x.name||'Player')}}</h1><p class="muted">${{E(x.nationality||'')}} · Age ${{E(x.age??'—')}}</p></div><div></div></div></div><div class="card"><h2>Player Statistics</h2><pre style="white-space:pre-wrap">${{E(JSON.stringify(stats,null,2))}}</pre></div>`;
 }}catch(e){{page.innerHTML=`<div class="card"><h2>Player data unavailable</h2><p>${{E(e.message)}}</p></div>`}}
}}
...
async def dedicated_match_stats_page(sport:str,match_id:str,league:str=Query("")):
    ...
    return HTMLResponse(_standalone_sports_page("match",match_id,sport,league),headers={"Cache-Control":"no-store"})
...
async def multisport_finished(sport:str,limit:int=Query(7,ge=1,le=7)):
    sport=sport.lower().strip()
    if sport not in {"rugby","cricket"}:raise HTTPException(400,"Supported sports: rugby, cricket")
    now=datetime.now(timezone.utc)
    # Finished games were previously searched only ~12 hours backwards.
    # Pull seven days and use OddsPapi's stable statusId=2 contract.
    rows=await _oddspapi_sport_range(sport,now-timedelta(days=7),now+timedelta(minutes=5),2)
    done=[x for x in rows if x.get("statusId")==2 or _multi_outcome(x) or str(x.get("status") or "").lower() in {"finished","ended","complete","completed","full time"}]
    done.sort(key=lambda x:str(x.get("date") or ""),reverse=True)
    return {"sport":sport,"matches":done[:limit],"count":min(len(done),limit),"source":"OddsPapi","windowDays":7}
...
async def multisport_predictions(sport:str,limit:int=Query(12,ge=1,le=30)):
    ...
    return {"sport":sport,"predictions":result,"count":len(result),"source":"Kasi prediction engine",
            "upcomingWindowDays":7,"historyWindowDays":14}
...
async def multisport_teams_players(sport:str,limit:int=Query(60,ge=1,le=200)):
    ...
    for x in rows:
        for side in ("home","away"):
            ...
            teams[str(tid)]={"id":tid,"name":name,"badge":x.get(side+"Logo") or "","flag":"","country":x.get("country") or "",
                             "sport":sport.title()}
    # OddsPapi fixtures provide team participants; player rosters are provider/competition dependent.
    return {"sport":sport,"teams":list(teams.values())[:limit],"players":[],"teamCount":min(len(teams),limit),
            "playerCount":0,"source":"OddsPapi","playersMessage":"Player roster data is not supplied by the active fixture feed for this sport."}
...
async def match_tip_stats(fixture_id:int):
    ...
    async def odds():
        ...
        return {}
    hr,ar,hh,oo=await asyncio.gather(recent(hid),recent(aid),h2h(),odds())
    def won(row,tid):
...
async def tip_stats_fast(fixture_id: int):
    ...
    try:
        data=await asyncio.wait_for(match_tip_stats(fixture_id),timeout=8.0)
        if isinstance(data,dict):
...
async def canonical_match_stats_page(fixture_id:str):
    return HTMLResponse(_standalone_sports_page("match",fixture_id,"football",""),headers={"Cache-Control":"no-store"})

@app.get("/teams/{team_id}",response_class=HTMLResponse)
async def canonical_team_page(team_id:str):
    return HTMLResponse(_standalone_sports_page("team",team_id,"football",""),headers={"Cache-Control":"no-store"})

@app.get("/players/{player_id}",response_class=HTMLResponse)
async def canonical_player_page(player_id:str):
    return HTMLResponse(_standalone_sports_page("player",player_id,"football",""),headers={"Cache-Control":"no-store"})
...
async def ssr_team(slug:str):
    ...
    return HTMLResponse(_standalone_sports_page("team",slug,"football",""),headers={"Cache-Control":"no-store"})
