...
async def sports_directory_search(q:str=Query(...,min_length=2,max_length=80),sport:str=Query("all")):
    """Global sports-data search. Never queries the News API."""
    q=q.strip(); ql=q.lower(); wanted=sport.strip().lower()
    sports=["football","rugby","cricket"] if wanted=="all" else [wanted]
    result={"query":q,"sport":wanted,"teams":[],"players":[],"competitions":[],"fixtures":[],"standings":[],"source":"sports providers/cache"}

    if "football" in sports:
        try:
            d=await directory_search(q=q,season=0)
            result["teams"] += [{**x,"sport":"football"} for x in d.get("teams",[])]
            result["players"] += [{**x,"sport":"football"} for x in d.get("players",[])]
        except Exception as exc: result.setdefault("errors",[]).append("football: "+str(exc))

    for sp in [x for x in sports if x in {"rugby","cricket"}]:
        try:
            d=await sports_directory(sp)
            result["teams"] += [{**x,"sport":sp} for x in d.get("teams",[]) if ql in str(x.get("name","")).lower()]
            result["players"] += [{**x,"sport":sp} for x in d.get("players",[]) if ql in str(x.get("name","")).lower()]
        except Exception as exc: result.setdefault("errors",[]).append(sp+": "+str(exc))

    # Search configured competition registry.
    result["competitions"]=[{**x,"type":"competition"} for x in COMPETITION_REGISTRY
        if (wanted=="all" or x.get("sport")==wanted) and ql in (str(x.get("name",""))+" "+str(x.get("country",""))).lower()][:20]

    # Search normalized current/upcoming/recent score feeds without news.
    async def scores(sp):
        try:return await _sports_scores(sp,None,False,None)
        except Exception as exc:
            result.setdefault("errors",[]).append(sp+": "+str(exc));return []
    batches=await asyncio.gather(*[scores(sp) for sp in sports])
    for sp,rows in zip(sports,batches):
        for g in rows:
            blob=" ".join(str(g.get(k,"")) for k in ("home","away","league","country")).lower()
            if ql in blob: result["fixtures"].append({**g,"sport":sp})
    result["teams"]=result["teams"][:30];result["players"]=result["players"][:30]
    result["fixtures"]=result["fixtures"][:40]
    result["count"]=sum(len(result[k]) for k in ("teams","players","competitions","fixtures","standings"))
    return result
...
async def cricket_predictions(date:str=Query(""),limit:int=Query(20,ge=1,le=50)):
    ...
    return {"sport":"cricket","count":len(out[:limit]),"predictions":out[:limit],
            "state":"ok" if out else "no-games","source":"normalized cricket feed + subscribed bookmaker odds"}
...
async def platform_status():
    ...
    return {
        ...
        "cricketProvider": True,  # normalized ESPN cricket feed; odds require ODDS_API_KEY
...
async def dedicated_match_stats_page(sport: str, match_id: str, league: str = Query("")):
    ...
    if sport not in {"football","rugby","cricket"}:
        raise HTTPException(404,"Match not found")
    mid=(match_id or "").strip()
    if not mid or not re.fullmatch(r"[A-Za-z0-9_.:-]{1,80}",mid):
        raise HTTPException(404,"Match not found")
    suffix=f"?league={urllib.parse.quote(league,safe='')}" if league else ""
    # The existing SEO renderer supplies the shell; the hash route tells the client
    # to open only the Match Statistics view for this match.
    return HTMLResponse(
        await _render_seo_document(f"/#match-stats/{sport}/{mid}{suffix}"),
