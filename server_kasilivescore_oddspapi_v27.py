...
ODDSPAPI_WS_BASE = os.getenv("ODDSPAPI_WS_BASE", "wss://api.oddspapi.io/v4/ws")
...
_oddspapi_ws_state = {
    "connected": False, "lastMessageAt": None, "lastError": None,
    "messages": 0, "reconnects": 0,
}
...
async def _oddspapi_ws_loop():
    """Continuously merge OddsPAPI one-way partial updates into the odds cache."""
    if not ODDSPAPI_ENABLED or not ODDSPAPI_API_KEY:
        return
    if websockets is None:
        ...
        return
    delay = 2
    url = f"{ODDSPAPI_WS_BASE}?apiKey={ODDSPAPI_API_KEY}"
    while True:
        try:
            async with websockets.connect(url, ping_interval=20, ping_timeout=20, close_timeout=5) as ws:
                _oddspapi_ws_state.update({"connected": True, "lastError": None})
                delay = 2
                async for raw in ws:
                    try:
                        payload = json.loads(raw)
                        if not isinstance(payload, dict) or not payload.get("fixtureId"):
                            continue
                        # Keep only selected bookmakers; metadata/scores still merge.
                        _oddspapi_store(payload)
                        _oddspapi_ws_state["messages"] += 1
                        _oddspapi_ws_state["lastMessageAt"] = datetime.now(timezone.utc).isoformat()
                    except Exception as exc:
                        log.warning("OddsPAPI WebSocket message ignored: %s", exc)
        except asyncio.CancelledError:
            raise
        except Exception as exc:
            _oddspapi_ws_state.update({"connected": False, "lastError": str(exc)})
            _oddspapi_ws_state["reconnects"] += 1
            log.warning("OddsPAPI WebSocket disconnected: %s; reconnecting in %ss", exc, delay)
...
async def lifespan(app: FastAPI):
    ...
    oddspapi_task = asyncio.create_task(_oddspapi_ws_loop())
...
async def _youtube_channel_id(handle: str) -> str:
    ...
    try:
        c = await _sports_client()
        r = await c.get(url,follow_redirects=True,timeout=httpx.Timeout(8,connect=4),
...
async def _build_video_pool() -> list[dict]:
...
async def oddspapi_health():
    return {
        ...
        "strategy": "API-Football primary -> OddsPAPI fallback -> cache -> prediction engine",
    }
