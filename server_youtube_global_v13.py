...
# ---------------------------------------------------------------------------
# YOUTUBE SPORTS VIDEO FEEDS — no API key required
# Uses YouTube's supported channel Atom feed:
# https://www.youtube.com/feeds/videos.xml?channel_id=CHANNEL_ID
# ---------------------------------------------------------------------------
_YT_CHANNEL_ID_CACHE = TTLCache(maxsize=64, ttl=86400)

_YT_SPORT_CHANNELS = {
    # Balanced worldwide mix. No country or publisher receives priority.
    "football": [
    ...
    "cricket": [
        ("ICC", "@ICC"),
        ("Cricket South Africa", "@cricketsatv"),
        ("SuperSport", "@supersport"),
    ],
}

async def _youtube_channel_id(handle: str) -> str:
    """Resolve a YouTube @handle to its UC... channel id without Data API."""
    ...
    try:
        ...
        for pat in patterns:
            ...
            if m:
                cid = m.group(1)
                _YT_CHANNEL_ID_CACHE[handle.lower()] = cid
                return cid
    except Exception as exc:
        log.debug("YouTube handle resolution failed %s: %s", handle, exc)
    return ""

async def _youtube_channel_feed(label: str, handle: str, limit: int = 8) -> list[dict]:
    cid = await _youtube_channel_id(handle)
    if not cid:
        return []

    feed_url = "https://www.youtube.com/feeds/videos.xml?channel_id=" + urllib.parse.quote(cid)
    try:
        c = await _sports_client()
        r = await c.get(
            feed_url,
            timeout=httpx.Timeout(8, connect=4),
            headers={
                "User-Agent":"Mozilla/5.0 (compatible; KasiSportsNews/1.0)",
        ...
        for entry in root.findall("atom:entry", ns)[:limit]:
            ...
            if group is not None:
                ...
                if t is not None and t.attrib.get("url"):
                    thumb = t.attrib["url"]
...
async def sports_videos(
    ...
    """Recent sports videos from official YouTube channel Atom feeds.

    Independent from API-Football and does not require a YouTube Data API key.
    """
    sport = sport.lower().strip()
    channels = _YT_SPORT_CHANNELS.get(sport, _YT_SPORT_CHANNELS["football"])

    results = await asyncio.gather(
        *[_youtube_channel_feed(label, handle, max(6, limit)) for label, handle in channels],
        return_exceptions=True,
    )
