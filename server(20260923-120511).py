...
def auth_login(p: AuthPayload, request: Request):
    """Authenticate a user with stage-safe diagnostics. Never logs passwords/hashes."""
    stage = "start"
    c = None
    try:
        stage = "rate_limit"
        ...
        stage = "identifier"
        identifier = (p.identifier or p.email or "").strip()
        if not identifier or not p.password:
            raise HTTPException(400, "Email/username/phone and password are required")
        ...
        stage = "user_lookup"
        row = c.execute(
            "SELECT id,email,password_hash,pro,username,phone,"
            "COALESCE(role,'user'),COALESCE(status,'active') "
            "FROM users WHERE lower(email)=lower(?) OR lower(username)=lower(?) OR phone=?",
            (identifier, identifier, phone),
        ).fetchone()

        if not row:
            raise HTTPException(401, "Invalid email, username/phone or password")
        if str(row[7] or "active").lower() != "active":
            raise HTTPException(403, "Account disabled")

        stage = "password_hash"
        password_hash = str(row[2] or "")
        if "$" not in password_hash:
            log.error("AUTH LOGIN failed stage=%s user_id=%s reason=invalid_password_hash_format", stage, row[0])
            raise HTTPException(500, "Account password data needs to be reset")
        salt, stored = password_hash.split("$", 1)
        try:
            calculated = _pw(p.password, salt).split("$", 1)[1]
        except (ValueError, TypeError) as exc:
            log.error("AUTH LOGIN failed stage=%s user_id=%s reason=invalid_password_hash", stage, row[0])
            raise HTTPException(500, "Account password data needs to be reset") from exc
        if not hmac.compare_digest(calculated, stored):
            raise HTTPException(401, "Invalid email, username/phone or password")

        stage = "admin_entitlement"
        configured_admin = os.getenv(
            "ADMIN_EMAIL", os.getenv("PAYMENT_ADMIN_EMAIL", "jbatuma@yahoo.com")
        ).strip().lower()
        is_configured_admin = bool(
            configured_admin and str(row[1] or "").strip().lower() == configured_admin
        )
        ...
        if is_configured_admin:
            c.execute(
                "UPDATE users SET last_login=?, pro=1, role='admin', status='active' WHERE id=?",
                (now_iso, row[0]),
            )
        else:
            c.execute("UPDATE users SET last_login=? WHERE id=?", (now_iso, row[0]))
        c.commit()

        stage = "token"
        token = _token(row[0])
        log.info("AUTH LOGIN success user_id=%s role=%s pro=%s", row[0], effective_role, effective_pro)
        return {
            "token": token,
            "email": row[1],
            "username": row[4],
        ...
        log.exception("AUTH LOGIN failed stage=%s error=%s", stage, type(exc).__name__)
        raise HTTPException(500, f"Login failed at stage: {stage}") from exc
    finally:
        if c is not None:
            try:
                c.close()
...
async def _render_seo_document(path: str) -> str:
    ...
    # v216: SEO metadata/schema stay in <head>; the old visible SSR article is removed.
    _SSR_CACHE[key]=html
    return html
...
async def serve_manifest():
    """Always return a valid PWA manifest; malformed/missing disk files cannot break the browser console."""
    manifest = {
        "name": "Kasi Sports News",
        "short_name": "Kasi Sports",
        "description": "Live scores, sports news, standings and predictions.",
        "start_url": "/",
        "scope": "/",
        "display": "standalone",
    ...
    if f.exists():
        try:
            disk = json.loads(f.read_text(encoding="utf-8-sig"))
            if isinstance(disk, dict):
                manifest.update(disk)
        except Exception:
            pass
