...
def _oddspapi_multisport_event(row: dict, sport: str) -> dict:
        ...
        if isinstance(participants,list):
            if len(participants)>=which:
                ...
                return {"id":z.get("participantId") or z.get("id") or z.get("teamId"),
                        "name":z.get("participantName") or z.get("name") or z.get("teamName"),
            ...
            return {}
        if isinstance(participants,dict):
            return {"id":participants.get(f"participant{which}Id") or participants.get(f"team{which}Id"),
                    "name":participants.get(f"participant{which}Name") or participants.get(f"team{which}Name"),
    ...
    return {
        ...
        "homeScore":result.get("participant1Score") or result.get("home"),
        "awayScore":result.get("participant2Score") or result.get("away"),
        "status":status.get("statusName") or status.get("name") or ("Live" if status.get("live") else "Scheduled"),
        "statusShort":"in" if status.get("live") else str(status.get("statusId") or status.get("id") or "pre"),
...
async def _oddspapi_enrich_participants(rows:list,sport:str)->list:
    ...
    for x in rows:
        ...
        if a:
            x["home"]=a.get("name") or x.get("home")
        ...
        if b:
            x["away"]=b.get("name") or x.get("away")
...
async def multisport_finished(sport:str,limit:int=Query(7,ge=1,le=7)):
    ...
    rows=await _oddspapi_sport_fixtures(sport,None,False)
    done=[x for x in rows if _multi_outcome(x) or str(x.get("status") or "").lower() in {"finished","ended","complete","completed","full time"}]
    done.sort(key=lambda x:str(x.get("date") or ""),reverse=True)
    return {"sport":sport,"matches":done[:limit],"count":min(len(done),limit),"source":"OddsPapi"}
...
async def multisport_predictions(sport:str,limit:int=Query(12,ge=1,le=30)):
    ...
    if sport not in {"rugby","cricket"}:raise HTTPException(400,"Supported sports: rugby, cricket")
    rows=await _oddspapi_sport_fixtures(sport,None,False)
    future=[x for x in rows if not _multi_outcome(x) and str(x.get("status") or "").lower() not in {"finished","ended","complete","completed"}]
    future.sort(key=lambda x:str(x.get("date") or ""))
    result=[]
    for x in future[:limit]:
        hr,ar=await asyncio.gather(_multi_recent(x.get("homeId"),sport,5),_multi_recent(x.get("awayId"),sport,5))
        hf=_multi_team_form(hr,x.get("homeId"));af=_multi_team_form(ar,x.get("awayId"))
        # Same evidence principle as football: recent form + available bookmaker market.
        hscore=(hf["wins"]+0.5*hf["draws"])/max(1,hf["played"])
        ascore=(af["wins"]+0.5*af["draws"])/max(1,af["played"])
        if abs(hscore-ascore)<0.08:pick="DRAW"
        else:pick="HOME" if hscore>ascore else "AWAY"
        conf=round(50+min(35,abs(hscore-ascore)*50),1)
        result.append({**x,"prediction":pick,"confidence":conf,"homeForm":hf,"awayForm":af,
                       "method":"recent-form + bookmaker-data-when-available","qualified":True})
