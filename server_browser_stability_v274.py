...
<text x="104" y="435" fill="#9fb0c3" font-family="Arial,Helvetica,sans-serif" font-size="28">kasilivescore.com</text>
    ...
    return Response(content=svg, media_type="image/svg+xml", headers={"Cache-Control":"public, max-age=86400, s-maxage=604800"})
...
def google_robots_txt():
    ...
    return Response(content=body, media_type="text/plain; charset=utf-8", headers={"Cache-Control":"no-cache"})
...
async def finished_games(hours:int=Query(24,ge=1,le=168),league:str=Query("ALL"),limit:int=Query(100,ge=1,le=200),days:Optional[int]=Query(None)):
    """All completed football matches in the rolling window ending now. Lightweight; Stats stays on-demand."""
    now=datetime.now(timezone.utc)
    if days is not None: hours=max(1,min(168,int(days)*24))
    cutoff=now-timedelta(hours=hours)
    # Scan every calendar date touched by the rolling UTC window.
    scan_start=cutoff.date()
    scan_end=now.date()
    ...
    selected=rows[:limit]
    return {"count":len(selected),"matches":selected,"hours":hours,
...
def _widget_match_record(row:dict, sport:str="football")->dict:
    ...
    if x.get("homeScore") is None:
        x["homeScore"]=x.get("homeGoals")
    if x.get("awayScore") is None:
        x["awayScore"]=x.get("awayGoals")
    return x
...
async def _football_match_details_build(fid:int):
    ...
    payload={"sport":"football","matchId":fid,"match":f,
             "statistics":stats.get("statistics",[]),"teams":lineups.get("teams",[]),
             "events":events.get("events",[]),"insights":insights,
             "players":players.get("response",[]),"odds":odds_detail,
             "source":"API-Football","available":bool(f),
             "partialAvailable":bool(f or stats.get("statistics") or lineups.get("teams") or events.get("events") or players.get("response") or insights),
...
async def sport_match_details(sport:str,match_id:str,league:str=Query(""),refresh:int=Query(0,ge=0,le=1)):
    ...
    # Existing subscribed multisport provider path remains unchanged.
    ...
    return {"sport":sport,"matchId":f.get("id"),"match":f,
            ...
            "source":"OddsPapi","updatedAt":datetime.now(timezone.utc).isoformat()}
...
async def sports_news(
    ...
    selected=verified[offset:offset+limit]
    # Vet new candidates only in background. Old verified data stays visible until replacement passes.
    warming=not bool(verified)
    asyncio.create_task(_refresh_verified_news_snapshot(pool,max(24,limit+offset)))
...
def _dashboard_snapshot_get(key):
    try:
        c=sqlite3.connect(_SHARED_SNAPSHOT_DB)
        c.execute("CREATE TABLE IF NOT EXISTS snapshots(k TEXT PRIMARY KEY,payload TEXT NOT NULL,updated REAL NOT NULL)")
        r=c.execute("SELECT payload,updated FROM snapshots WHERE k=?",(key,)).fetchone();c.close()
        if not r:return None
        d=json.loads(r[0]);d["updated_at"]=r[1];return d
    except Exception:return None
...
def _dashboard_snapshot_put(key,payload):
    try:
        c=sqlite3.connect(_SHARED_SNAPSHOT_DB)
        c.execute("CREATE TABLE IF NOT EXISTS snapshots(k TEXT PRIMARY KEY,payload TEXT NOT NULL,updated REAL NOT NULL)")
        c.execute("INSERT INTO snapshots(k,payload,updated) VALUES(?,?,?) ON CONFLICT(k) DO UPDATE SET payload=excluded.payload,updated=excluded.updated",
...
async def _refresh_verified_news_snapshot(items,limit=24):
    """Validate candidates in background; replace last-good snapshot only with a non-empty verified set."""
    ...
    async with _NEWS_VERIFY_LOCK:
        try:
            ...
            if qualified:
                ...
                if not current or len(merged)>=min(len(current),limit):
                    _NEWS_VERIFIED_SNAPSHOT["items"]=merged
...
.ks-lead{{font-size:18px;line-height:1.65}}.ks-original{{margin-top:28px;padding-top:20px;border-top:1px solid var(--line)}}
</style></head><body>
...
async def _render_seo_document(path: str) -> str:
    ...
    html=(BASE_DIR/"index.html").read_text(encoding="utf-8"); public=KASI_PUBLIC_ORIGIN; canonical=f"{public}{path}"
...
async def v227_cache_headers(request: Request, call_next):
    ...
    if p.startswith("/live") or p.startswith("/sports/football/live") or p.startswith("/api/live-scores-with-odds"):
        ...
        # v274 stability: never let a normal browser/edge retain an obsolete app shell.
...
async def root_webapp(): return HTMLResponse(await _render_seo_document("/"),headers={"Cache-Control":"no-store, max-age=0"})
...
function lineups(ts){{return ts.length?ts.map(t=>`<div class="card"><h3>${{E(t.team?.name||'Team')}} <span class="pill">${{E(t.formation||'Formation —')}}</span></h3><div class="tiny muted">Coach: ${{E(t.coach?.name||'—')}}</div>${{(t.players||[]).map(p=>`<a class="person" href="${{playerUrl(p.id)}}"><span class="pill">${{E(p.number??'—')}}</span><div><b>${{E(p.name||'Player')}}</b><div class="tiny muted">${{E(p.pos||'')}} · ${{p.starter?'Starting XI':'Substitute'}}</div></div><span>›</span></a>`).join('')}}</div>`).join(''):'<div class="empty">No lineups returned.</div>'}}
...
function intelligence(i){{i=i||{{}};const f=i.form||{{}},fh=f.home||{{}},fa=f.away||{{}},st=i.standings||{{}},h2=i.h2h||{{}},n=i.next5||{{}},ts=i.teamStatistics||{{}};
 const formRows=x=>(x||[]).map(r=>`<div class="event"><div class="minute">${{E(r.result||'—')}}</div><div><b>${{E(r.home||'')}} ${{E(r.score||'')}} ${{E(r.away||'')}}</b><div class="tiny muted">${{localDate(r.date)}}${{(r.scorers||[]).length?' · Scorers: '+E((r.scorers||[]).join(', ')):''}}</div></div></div>`).join('')||'<div class="empty">No recent form returned.</div>';
 const nextRows=x=>(x||[]).map(r=>`<div class="event"><div class="minute">NEXT</div><div><b>${{E(r.home||'')}} vs ${{E(r.away||'')}}</b><div class="tiny muted">${{localDate(r.date)}}</div></div></div>`).join('')||'<div class="empty">No upcoming team fixtures returned.</div>';
 const teamStat=x=>{{const fx=x?.fixtures||{{}},go=x?.goals||{{}};return `<div class="grid3"><div class="metric"><span class="tiny muted">Played</span><b>${{E(fx.played?.total??'—')}}</b></div><div class="metric"><span class="tiny muted">Wins</span><b>${{E(fx.wins?.total??'—')}}</b></div><div class="metric"><span class="tiny muted">Goals</span><b>${{E(go.for?.total?.total??'—')}} / ${{E(go.against?.total?.total??'—')}}</b></div></div>`}};
 return `<div class="grid2"><div class="card"><h3>${{E(fh.team?.name||'Home')}} · Last 10</h3>${{formRows(fh.last10)}}</div><div class="card"><h3>${{E(fa.team?.name||'Away')}} · Last 10</h3>${{formRows(fa.last10)}}</div></div>
 <div class="grid2"><div class="card"><h3>League position</h3><p>${{E(fh.team?.name||'Home')}}: <b>#${{E(st.home?.rank??'—')}}</b></p><p>${{E(fa.team?.name||'Away')}}: <b>#${{E(st.away?.rank??'—')}}</b></p></div><div class="card"><h3>Head to head</h3><p class="tiny muted">${{E(h2.played??0)}} meetings · Home wins ${{E(h2.homeWins??0)}} · Draws ${{E(h2.draws??0)}} · Away wins ${{E(h2.awayWins??0)}} · BTTS ${{E(h2.bttsPct??0)}}% · O2.5 ${{E(h2.over25Pct??0)}}%</p>${{(h2.meetings||[]).slice(0,10).map(x=>`<div class="event"><div class="minute">H2H</div><div><b>${{E(x.home||'')}} ${{E(x.score||'')}} ${{E(x.away||'')}}</b><div class="tiny muted">${{localDate(x.date)}}</div></div></div>`).join('')||'<div class="empty">No H2H returned.</div>'}}</div></div>
 ...
 <div class="grid2"><div class="card"><h3>Team statistics · ${{E(fh.team?.name||'Home')}}</h3>${{teamStat(ts.home)}}</div><div class="card"><h3>Team statistics · ${{E(fa.team?.name||'Away')}}</h3>${{teamStat(ts.away)}}</div></div>`}}
...
async function load(force=false){{try{{const qs=new URLSearchParams();if(LEAGUE)qs.set('league',LEAGUE);if(force)qs.set('refresh','1');const d=await J('/sports/'+encodeURIComponent(SPORT)+'/match/'+encodeURIComponent(ID)+(qs.toString()?'?'+qs.toString():'')),m=d.match||{{}};if(SPORT!=='football'){{page.innerHTML=`<div class="card"><h1>${{E(m.home||m.homeTeam||'Home')}} vs ${{E(m.away||m.awayTeam||'Away')}}</h1><div class="score">${{E(m.homeScore??'—')}} – ${{E(m.awayScore??'—')}}</div><p class="muted">${{E(m.league||SPORT)}} · ${{E(m.status||'')}}</p></div><div class="card"><h2>Sport-specific match data</h2><pre style="white-space:pre-wrap">${{E(JSON.stringify(d.statistics?.sportSpecific||d.statistics||{{}},null,2))}}</pre></div>`;return}}const ts=m.teams||{{}},ev=d.events||[],st=d.statistics||[],lu=d.teams||[],pl=d.players||[],home=ts.home||{{}},away=ts.away||{{}},fx=m.fixture||{{}},lg=m.league||{{}};page.innerHTML=`<div class="card hero"><a class="team" href="/teams/${{encodeURIComponent(home.id||'')}}">${{home.logo?`<img src="${{E(home.logo)}}">`:''}}<div class="team-name">${{E(home.name||'Home')}}</div><div class="scorers">${{scorerLines(ev,home.id)||'&nbsp;'}}</div></a><div><div class="tiny muted">${{E(lg.country||'')}} · ${{E(lg.name||'Football')}}</div><div class="score">${{E(m.goals?.home??'—')}} – ${{E(m.goals?.away??'—')}}</div><div class="status">${{E(fx.status?.long||'Match')}}</div><div class="tiny muted">${{localDate(fx.date)}}<br>${{E(fx.venue?.name||'Venue —')}}</div></div><a class="team" href="/teams/${{encodeURIComponent(away.id||'')}}">${{away.logo?`<img src="${{E(away.logo)}}">`:''}}<div class="team-name">${{E(away.name||'Away')}}</div><div class="scorers">${{scorerLines(ev,away.id)||'&nbsp;'}}</div></a></div><div class="tabs"><button class="active" data-p="overview">Overview</button><button data-p="timeline">Timeline</button><button data-p="stats">Statistics</button><button data-p="lineups">Lineups</button><button data-p="players">Player Stats</button><button data-p="intel">Match Intelligence</button><button data-p="odds">Odds / Prediction</button></div><section class="panel active" id="overview"><div class="grid3"><div class="metric"><span class="muted tiny">Venue</span><b>${{E(fx.venue?.name||'—')}}</b><span class="tiny muted">${{E(fx.venue?.city||'')}}</span></div><div class="metric"><span class="muted tiny">Referee</span><b>${{E(fx.referee||'—')}}</b></div><div class="metric"><span class="muted tiny">Round</span><b>${{E(lg.round||'—')}}</b></div></div><div class="card"><h2>Key Events</h2>${{timeline(ev.filter(x=>['goal','card'].includes(String(x.type||'').toLowerCase())).slice(0,14))}}</div><div class="card"><h2>Match Statistics</h2>${{statRows(st)}}</div></section><section class="panel" id="timeline"><div class="card"><h2>Match Timeline</h2>${{timeline(ev)}}</div></section><section class="panel" id="stats"><div class="card"><h2>Detailed Statistics</h2>${{statRows(st)}}</div></section><section class="panel" id="lineups"><div class="grid2">${{lineups(lu)}}</div></section><section class="panel" id="players"><div class="card"><h2>Player Match Data</h2><p class="tiny muted">Click a player for the full profile.</p>${{playerStats(pl)}}</div></section><section class="panel" id="intel">${{intelligence(d.insights)}}</section><section class="panel" id="odds">${{odds(d.odds)}}</section>`;wireTabs();if(!force&&(d.cached||d.refreshing))setTimeout(()=>load(true),350)}}catch(e){{page.innerHTML=`<div class="card"><h2>Match data temporarily unavailable</h2><p class="muted">The match service did not return usable data. Please retry.</p><button onclick="location.reload()">Retry</button></div>`}}}}load();
</script></body></html>"""
...
async function J(u){{const c=new AbortController(),t=setTimeout(()=>c.abort(),6500);try{{const r=await fetch(u,{{signal:c.signal,headers:{{Accept:'application/json'}},cache:'no-store'}});if(!r.ok)throw Error('HTTP '+r.status);return await r.json()}}finally{{clearTimeout(t)}}}}
...
async def dedicated_match_stats_page(sport:str,match_id:str,league:str=Query("")):
    ...
    return HTMLResponse(_standalone_match_stats_page(match_id,sport,league),headers={"Cache-Control":"no-store"})
...
async def _oddspapi_fixture_scores(fixture_id:str)->dict:
    ...
    try:
        ...
        return {"fixtureId":fid,"periods":periods,"latest":latest,"available":bool(periods)}
    except Exception as exc:
...
async def canonical_match_stats_page(fixture_id:str):
    return HTMLResponse(_standalone_match_stats_page(fixture_id,"football",""),headers={"Cache-Control":"no-store"})
