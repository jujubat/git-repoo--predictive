...
def _oddspapi_multisport_event(row: dict, sport: str) -> dict:
    ...
    try:
        ...
        dt=str(row.get("trueStartTime") or row.get("startDate") or "") or None
    return {
        "id":str(row.get("fixtureId") or row.get("id") or ""),
        "sport":sport,
        "league":tournament.get("tournamentName") or sport.title(),
        "leagueSlug":str(tournament.get("tournamentId") or ""),
        "country":tournament.get("categoryName") or "",
        "date":dt,
        "home":participants.get("participant1Name") or "Home",
        "away":participants.get("participant2Name") or "Away",
        "homeId":participants.get("participant1Id"),
        "awayId":participants.get("participant2Id"),
        "homeLogo":"",
        "awayLogo":"",
        "homeScore":result.get("participant1Score"),
        "awayScore":result.get("participant2Score"),
        "status":status.get("statusName") or ("Live" if status.get("live") else "Scheduled"),
        "statusShort":"in" if status.get("live") else str(status.get("statusId") or "pre"),
        "elapsed":0,
        "venue":(row.get("venue") or {}).get("venueName") or "",
        "source":"OddsPapi",
        "providerFixtureId":str(row.get("fixtureId") or ""),
    }
...
async def sports_match_detail(sport: str, match_id: str, league: str = Query("")):
    ...
    if sport == "football":
        ...
        stats,lineups,events,insights=await asyncio.gather(
            fixture_stats(fid),fixture_lineups(fid),fixture_events(fid),fixture_insights(fid),
            return_exceptions=True
        )
        ...
        if not f:
            return {"sport":"football","matchId":fid,"match":{},"statistics":[],"teams":[],"events":[],
                    "insights":{},"source":"API-Football","available":False,
                    "error":"Fixture was not returned by the football provider for this fixture ID.",
                    "updatedAt":datetime.now(timezone.utc).isoformat()}
        return {"sport":"football","matchId":fid,"match":f,"statistics":stats.get("statistics",[]),
                "teams":lineups.get("teams",[]),"events":events.get("events",[]),"insights":insights,
...
</style></head><body><main><div class="top"><a class="back" href="/">← Back to Home</a><b>Kasi Sports News</b></div>
