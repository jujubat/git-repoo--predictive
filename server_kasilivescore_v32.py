...
async def _oddspapi_get(path: str, params: Optional[dict] = None) -> Any:
    ...
    async with httpx.AsyncClient(timeout=12.0) as client:
        ...
        if response.is_error:
            # Never expose the request URL because apiKey is a query parameter.
            detail = ""
            try:
                body = response.json()
...
async def _oddspapi_odds_for_afoot(afoot_fixture_id: Any, meta: dict, refresh: bool = False) -> tuple[dict, Optional[str]]:
    ...
    if rows:
        # /v4/odds is queried only for the matched fixtureId.
...
async def oddspapi_diagnostics():
    ...
    odds_rows = []
    sample_fixture_id = (fixtures[0].get("fixtureId") or fixtures[0].get("id")) if fixtures else None
    if sample_fixture_id:
        ...
        report["errors"].append("No OddsPAPI fixtureId available for odds test")
