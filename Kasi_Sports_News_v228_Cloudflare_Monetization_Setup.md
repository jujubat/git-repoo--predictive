...
## v228 visible commercial layout

Desktop inventory:
- top leaderboard
- responsive ad after Live
- responsive ad after Fixtures
- responsive ad after Predictions
- right rail 300x250-style slot
- right rail direct sponsor slot
- right rail tall 300x600-style slot
- right rail affiliate module
- bottom direct sponsor slot

The right rail is sticky on desktop. Below 900px it is removed and replaced by mobile responsive sponsor/affiliate placements so scores remain readable.

Optional Render environment variables after AdSense provides real slot IDs:
`ADSENSE_SLOT_LEADERBOARD=...`
`ADSENSE_SLOT_IN_CONTENT=...`
`ADSENSE_SLOT_RIGHT_RECTANGLE=...`
`ADSENSE_SLOT_RIGHT_SKYSCRAPER=...`
`ADSENSE_SLOT_MOBILE=...`

The layout intentionally uses placeholders until real AdSense/sponsor/affiliate identifiers are configured. Never use fake publisher or slot IDs.
