...
def _news_rank(item: dict, country: str) -> tuple:
    ...
    countries={str(x).upper() for x in item.get("countries",[])}
    # Country relevance first; freshness second; valid photography breaks ties.
    local=2 if c and c in countries else 1 if not countries else 0
    image=1 if item.get("imageAvailable") else 0
    return (local,_news_iso_value(item.get("published")),image)
...
async def sports_news(
    ...
    if market_query:
        ...
        if market is None:
            ...
            for row in rows:
                ...
                x["originalUrl"]=target;x["description"]=x.get("summary") or ""
                market.append(x)
    ...
    return {
        ...
        "items":selected,
        "country":visitor_country,"countryMethod":auto.get("method","general"),
        "fallback":"country-to-international","source":"Approved multi-source news pool",
        "updatedAt":datetime.now(timezone.utc).isoformat(),
    }
...
async def sports_news_bulletin(request: Request, limit: int = Query(5,ge=1,le=8), country: str = Query("")):
    ...
    return {**data,"items":(data.get("items") or [])[:limit],"count":min(limit,len(data.get("items") or []))}
...
async def seo_news_article(slug: str, source: str = Query("")):
    ...
    article=(f'<main class="kasiscore--page ks-internal-news-page" style="max-width:980px;margin:28px auto;padding:20px"><nav aria-label="Breadcrumb" style="margin-bottom:18px"><a href="/">← Back to Kasi Sports News</a> <span style="opacity:.55">/ News</span></nav>{img_html}<div class="badge" style="display:inline-block;margin:4px 0 10px">SPORTS NEWS</div><h1 style="font-size:clamp(28px,5vw,52px);line-height:1.08;margin:8px 0 14px">{escape(title)}</h1><div class="sub" style="margin-bottom:20px">Source: <strong>{escape(publisher)}</strong></div><p style="font-size:17px;line-height:1.75;max-width:820px">{escape(desc)}</p><div style="margin-top:24px;padding-top:18px;border-top:1px solid rgba(128,128,128,.25)"><p class="sub">Kasi Sports News displays the article details and publisher-permitted summary inside the dashboard. The publisher remains the source of the complete copyrighted article.</p><a class="btn" href="{escape(publisher_url,quote=True)}" target="_blank" rel="noopener noreferrer" style="display:inline-block;margin-top:10px">Read Original Article</a></div></main><script type="application/ld+json">{json.dumps(schema,ensure_ascii=False)}</script>')
